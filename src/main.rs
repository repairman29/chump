...
pub(crate) fn normalize_discord_token(s: &str) -> String {
    let s = s.trim();
...
pub(crate) fn print_priority_hint(store: &gap_store::GapStore, target_id: &str, repo_root: &std::path::Path) {
...
pub(crate) fn extract_path_hints(text: &str, repo_root: &std::path::Path) -> Vec<std::path::PathBuf> {
...
/// env var.
pub(crate) fn list_required_contexts() -> Vec<String> {
...
async fn main() -> Result<()> {
    ...
    cmd::gap::run(&args).await?;

    cmd::cost::run(&args).await?;
...
pub(crate) fn default_acceptance_criteria(title: &str, domain: &str) -> Vec<String> {
