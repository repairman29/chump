...
async fn main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("fleet") {
        ...
        match subcmd {
            ...
            // INFRA-900: `chump fleet metrics [--window H] [--dry-run] [--json]`
            // Delegates to scripts/ops/fleet-metrics-snapshot.sh and emits
            // kind=fleet_metrics_snapshot to ambient.jsonl.
            "metrics" => {
                ...
                let mut cmd = std::process::Command::new("bash");
                cmd.arg(&metrics_sh);
                // Forward --window, --dry-run, --json flags directly.
                for arg in &args[3..] {
                    cmd.arg(arg);
                }
                let status = cmd.status().unwrap_or_else(|e| {
            ...
            _ => {
                ...
                eprintln!(
                    "  metrics     [--window H] [--dry-run] [--json]  -- emit kind=fleet_metrics_snapshot (INFRA-900)"
                );
                std::process::exit(2);
            }
        }
