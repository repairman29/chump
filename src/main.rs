...
pub(crate) fn route_stuck_gap(effort: &str, strikes: u32, model_ladder_len: u32) -> GapDiscipline {
...
pub(crate) fn preflight_claim_is_self(current_session: Option<&str>, claim_session: &str) -> bool {
...
async fn main() -> Result<()> {
    ...
    // INFRA-1965 slice 2: the `gap` command group (6,079 LOC) moved to
    // commands::gap. Pure move — see that module's header for the receipts.
    if let Some(r) = commands::gap::try_dispatch(&args).await {
        return r;
    }
