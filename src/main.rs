...
async fn main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("fleet") {
        ...
        match subcmd {
            ...
            // INFRA-900: metrics snapshot subcommand — delegates to
            // scripts/ops/fleet-metrics-snapshot.sh and forwards all args.
            "metrics" => {
                let metrics_sh = repo_root.join("scripts/ops/fleet-metrics-snapshot.sh");
                if !metrics_sh.exists() {
                    eprintln!(
                        "chump fleet metrics: script not found at {}",
                        metrics_sh.display()
                    );
                    std::process::exit(1);
                }
                let mut cmd = std::process::Command::new("bash");
                cmd.arg(&metrics_sh);
                // Forward any flags after `chump fleet metrics`
