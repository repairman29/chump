...
async fn main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("fleet") {
        ...
        match subcmd {
            ...
            // INFRA-900: chump fleet metrics — collect + emit fleet_metrics_snapshot.
            // Proxies to scripts/ops/fleet-metrics-snapshot.sh with pass-through of
            // remaining args (--window, --dry-run, --json).
            "metrics" => {
