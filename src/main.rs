...
async fn main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("fleet") {
        ...
        match subcmd {
            ...
            // and emit kind=fleet_metrics_snapshot to ambient.jsonl.
            "metrics" => {
                ...
                let mut cmd = std::process::Command::new("bash");
                cmd.arg(&metrics_sh);
                // Forward remaining flags (--window, --dry-run, --json, etc.)
            ...
            _ => {
                ...
                eprintln!("  metrics     [--window H] [--dry-run] [--json]  -- emit fleet_metrics_snapshot (INFRA-900)");
                std::process::exit(2);
            }
        }
    }
