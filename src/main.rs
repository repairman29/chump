...
async fn main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("fleet") {
        ...
        match subcmd {
            ...
            // from ambient.jsonl + state.db. Delegates to
            // scripts/ops/fleet-metrics-snapshot.sh.
            "metrics" => {
                let metrics_sh = repo_root.join("scripts/ops/fleet-metrics-snapshot.sh");
                if !metrics_sh.exists() {
                    eprintln!("chump fleet metrics: {} not found", metrics_sh.display());
                    std::process::exit(1);
                }
                let passthrough: Vec<String> = args.iter().skip(3).cloned().collect();
                let mut cmd = std::process::Command::new("bash");
            ...
            _ => {
                ...
                eprintln!(
                    "  metrics     [--window H] [--dry-run] [--json]  -- emit fleet_metrics_snapshot (INFRA-900)"
                );
                std::process::exit(2);
            }
        }
