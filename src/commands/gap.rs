//! INFRA-1965 (slice 2 of the main.rs decomposition, after INFRA-3289's slice 1):
//! dispatch for the `chump gap ...` command group.
//!
//! PURE MOVE — the body below is `main()`'s `gap` branch verbatim, unchanged.
//! It was 6,079 LOC, 39% of `async fn main()`, and 84 of the last 300 commits
//! touching main.rs landed inside it (the #2 merge-contention hotspot).
//!
//! Extraction was safe because the branch is near free-standing: zero references
//! to any `main()` local (flag_verbose / flag_debug / wants_help), a single `?`
//! operator, and a single `return Err`. Wrapping in an inner `run()` that keeps
//! `anyhow::Result<()>` preserves all 37 `return Ok(())` sites untouched, so no
//! control flow was rewritten.
//!
//! Business logic already lived in the `chump-gap-store` crate (54 `gap_store::`
//! calls, zero direct SQL); what moved here is the CLI surface — 566 print sites
//! across ~39 subcommands.
//!
//! ONE line differs from the original, forced by the signature change from
//! `main()`'s owned `Vec<String>` to this module's `&[String]`:
//!   `args.clone()` -> `args.to_vec()`  (same value, same semantics).
//! Everything else is byte-identical to the pre-extraction branch.

use crate::{
    default_acceptance_criteria, extract_path_hints, is_acceptance_criteria_vague, is_test_domain,
    is_vague_ac_entry, parse_duration_to_secs, preflight_claim_is_self, print_priority_hint,
    repo_path, route_stuck_gap, unix_ts, version, write_yaml_op_marker,
};
use chump_gap_store as gap_store;

/// Handle the `gap` command group. Returns `Some(result)` when `args[1] == "gap"`,
/// `None` to let the next dispatcher try.
pub async fn try_dispatch(args: &[String]) -> Option<anyhow::Result<()>> {
...
async fn run(args: &[String]) -> anyhow::Result<()> {
    ...
    match subcmd {
        ...
        "strike" => {
            let gap_id = args.get(3).cloned().unwrap_or_else(|| {
                eprintln!("Usage: chump gap strike <GAP-ID> [--backend <name>] [--reset] [--show]");
                eprintln!();
                eprintln!("Increments the per-gap failure strike counter (default backend");
                eprintln!("chump-local). Exits 10 when count >= CHUMP_DECOMPOSE_STRIKE_THRESHOLD");
        ...
        "route" => {
            let gap_id = args.get(3).cloned().unwrap_or_else(|| {
                ...
                eprintln!();
                eprintln!("Recommends the recovery discipline for a stuck gap from effort-size");
                eprintln!("x strikes: retry | escalate-model | decompose | flag-human. Read-only.");
                std::process::exit(2);
            });
