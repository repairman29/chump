...
//! In-process mistral token streaming (`CHUMP_MISTRALRS_STREAM_TEXT_DELTAS`) applies only to the tool-approval path; standard turns still show the final reply only — see docs/MISTRALRS_AGENT_POWER_PATH.md §5.
    ...
    let mut to_send = match latest_mabel_report() {
        ...
        None => {
            if is_mabel_bot {
                ...
                "No `logs/mabel-report-*.md` on this host. The **unified fleet report** is generated by **Mabel** on the Pixel. Ask Mabel **`!status`** in Discord to get the latest, or check `logs/mabel-report-*.md` on the Pixel directly. Mac hourly updates are disabled when `CHUMP_MABEL_DRIVES_FLEET=1` is set."
                    .to_string()
            }
        }
    };
