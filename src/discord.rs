...
// META-013: agent factory + system prompt extracted to dedicated modules.
// discord.rs now owns only Discord IO surface (event handler, run, build_agent,
...
fn local_model_preflight_failed_message() -> String {
    ...
    if base.contains(":8000") {
        return "Model server isn't responding (vLLM-MLX on port 8000). On this Mac, from the Chump repo run `./scripts/setup/restart-vllm-if-down.sh`, wait until `curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8000/v1/models` prints 200, then **restart the Chump bot** (`./run-discord.sh` or `./run-discord-full.sh`). Check `logs/vllm-mlx-8000.log` if it keeps failing. (You do **not** need to restart the Discord *app*.)".to_string();
    }
    ...
    "Model server isn't responding. **Mac:** for vLLM on 8000 run `./scripts/setup/restart-vllm-if-down.sh`; for Ollama run `ollama serve`, then restart the Chump bot. **Pixel (Mabel):** run `./start-companion.sh` (or start `llama-server`). Restart the **Chump bot process**, not the Discord client app.".to_string()
}
...
async fn run_one_discord_turn(
    ...
    let reply = if !tool_policy::tools_requiring_approval().is_empty() {
        match build_chump_agent_web_components(&channel_id.to_string(), None, false, false) {
            Ok(b) => {
