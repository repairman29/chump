...
async fn free_tier_ship(gap_id: &str, repo_root: &std::path::Path) -> Result<()> {
    // EFFECTIVE-312: the agent run pins CHUMP_REPO to the worktree so file
...
pub async fn execute_gap(gap_id: &str) -> Result<String> {
    ...
    if free_tier {
        ...
        // EFFECTIVE-312: file tools (read/patch/git_commit) resolve paths via
        // CHUMP_REPO — inherited as the MAIN checkout from worker env, so every
        // model patch and commit landed OUTSIDE the dispatch worktree (found
        // stranded in /root/Chump after 15 cycles of "zero-commit branch").
        // Point the tools at THIS worktree; the canonical state.db is still
        // resolved via main_checkout_root() (git-common-dir), unaffected.
        if let Ok(cwd) = std::env::current_dir() {
            std::env::set_var("CHUMP_REPO", &cwd);
            eprintln!(
                "[execute-gap] EFFECTIVE-312: tool root pinned to worktree {}",
                cwd.display()
            );
        }
