...
mod tests {
    ...
    // ── ACP per-session MCP tool tests ──

    #[test]
    fn session_mcp_tool_clone_and_fields() {
        let t = SessionMcpTool {
            name: "my_tool".to_string(),
            description: "does something".to_string(),
            input_schema: serde_json::json!({"type": "object"}),
            command: "/usr/bin/my-server".to_string(),
            cmd_args: vec!["--flag".to_string()],
        };
        let c = t.clone();
        assert_eq!(c.name, "my_tool");
        assert_eq!(c.cmd_args, vec!["--flag"]);
    }

    #[test]
    fn acp_proxy_tool_name_and_description() {
        let t = SessionMcpTool {
            name: "search".to_string(),
            description: "Search the web".to_string(),
            input_schema: serde_json::json!({}),
            command: "/usr/bin/search-server".to_string(),
            cmd_args: vec![],
        };
        let proxy = AcpMcpProxyTool::new(t);
        assert_eq!(proxy.name(), "search");
        assert_eq!(proxy.description(), "[ACP-MCP] Search the web");
    }

    #[test]
    fn acp_proxy_tool_schema_passthrough() {
        ...
        let proxy = AcpMcpProxyTool::new(t);
        assert_eq!(proxy.input_schema(), schema);
    }

    #[tokio::test]
    async fn discover_all_acp_tools_empty_servers() {
        let tools = discover_all_acp_tools(&[]).await;
        assert!(tools.is_empty());
    }

    #[tokio::test]
    async fn discover_all_acp_tools_bad_command_returns_empty() {
        // A command that doesn't exist should produce a warning + empty result, not a panic.
        let servers = vec![(
            "nonexistent".to_string(),
            "/nonexistent-binary-xyzzy-chump-test".to_string(),
            vec![],
        )];
        let tools = discover_all_acp_tools(&servers).await;
        assert!(tools.is_empty());
    }
}
