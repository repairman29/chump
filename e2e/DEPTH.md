...
| `chumpbench/` (6 YAML tracks) | manual / harness — not a Playwright suite, not in CI | scenario (outside D1–D4) | zero-touch OS-capability acceptance: CREATE ×3, COMPREHEND, RESCUE, FINISH tracks with self-contained grading commands | not regression tests and not run on any schedule — a track can rot green-less for weeks; grading is per-track bespoke (no shared harness assertions); no ledger of last pass/fail per track |
