...
This crate **does not** read `.env` and **does not** edit `docs/gaps.yaml` (no status mutations). Lease changes go through `chump claim` only.
...
| Tool | Role |
|------|------|
| `gap_preflight` | Run `chump gap preflight` for one or more gap IDs. |
| `gap_claim_lease` | Run `chump claim` (writes lease JSON under `CHUMP_LOCK_DIR` / `.chump-locks/`). |
