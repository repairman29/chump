...
// Naming convention (INFRA-1768): assert_* tests are static preflight tests; behavior_* tests are runtime CI-only tests.
// assert_* tests have no external dependencies (no filesystem, no network) and are safe to run in `chump preflight`;
// behavior_* tests require a full CI environment.
#[cfg(test)]
mod tests {
    use super::*;
    use std::io::Write;
