...
#[tokio::test]
#[serial]
async fn deregister_removes_record_from_kv_bucket() {
    let bucket = unique_bucket();
    ...
    let worker_id = format!("worker-{}", &Uuid::new_v4().to_string()[..8]);
    let presence = build_presence(&worker_id, Some("CREDIBLE-256".to_string()));
    client
        .register_worker_presence(&presence)
        .await
        .expect("register");
    ...
    assert!(
        after.is_none(),
        "record should be gone from the KV bucket after deregistration, not just marked terminal"
    );
