        ...
        "ambient — append-only structured telemetry on .chump-locks/ambient.jsonl\n\n\
         USAGE:\n\
           ...
           ambient rotate [--path PATH]\n\
                  Rotate ambient.jsonl if it exceeds the size threshold.\n\n\
           ambient --help | -h\n\
                  Show this message.\n\n\
         ENV:\n\
           CHUMP_AMBIENT_LOG    Override the ambient log path.\n\
           CHUMP_AMBIENT_MAX_MB Override rotation threshold in MB (default 10 MB).\n\
...
fn run_rotate(args: &[String]) -> Result<()> {
    ...
    let rotated = ambient_rotate::rotate_if_needed(&path);
    if rotated {
        println!("Rotated {}", path.display());
    } else {
        println!("No rotation needed for {}", path.display());
    }
    Ok(())
}
