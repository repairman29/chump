...
use chump_ambient_cli::ambient_rotate;
    ...
    eprintln!(
        "ambient — append-only structured telemetry on .chump-locks/ambient.jsonl\n\n\
         USAGE:\n\
           ...
           ambient rotate [--path PATH]\n\
                  Rotate ambient.jsonl if it exceeds the threshold.\n\
                  Threshold: CHUMP_AMBIENT_MAX_MB env (default 10 MB).\n\n\
           ambient --help | -h\n\
                  Show this message.\n\n\
         ENV:\n\
           ...
           CHUMP_AMBIENT_MAX_MB Rotation threshold in MB (default 10)."
    );
}
...
fn run_rotate(args: &[String]) -> Result<()> {
    let mut path_override: Option<PathBuf> = None;
    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            ...
            other => return Err(anyhow!("unknown flag for rotate: {other}")),
        }
    }
    ...
    if ambient_rotate::rotate_if_needed(&path) {
        println!("ambient.jsonl rotated (exceeded threshold)");
    } else {
        println!("ambient.jsonl below threshold, no rotation needed");
    }
    Ok(())
}
