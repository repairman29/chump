...
/// verdict in `commands::reachability` — one definition of "is this a test
/// file" shared by Gate 2 and the mechanical stub detector.
///
/// `pub`, not `pub(crate)`: this module has since been extracted out of the
/// `chump` binary into the `chump-verify` crate, and the consumer
/// (`src/commands/reachability.rs`, which reaches it through the `pub use`
/// re-export at src/main.rs:24) now lives in a different crate. `pub(crate)`
/// was correct while both sides were one crate and stopped compiling the
