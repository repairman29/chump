                ...
                println!(
                    "Usage: chump claim <GAP-ID> [--paths CSV] [--session ID] [--no-doctor] [--no-import] [--force-recover]\n\n\
                     ...
                     Options:\n  \
                       ...
                       --force-overlap  Override hot-file (INFRA-1394) / lease path-set (INFRA-1604) collision block; warning still emitted\n  \
...
pub fn run_claim(args: ClaimArgs) -> Result<ClaimReport> {
    ...
    //
    // Structural successor to the 5.6 AC-text heuristic above: computes the
    // actual set intersection of {this claim's declared paths[]} ∩ {each
    // sibling lease's declared paths[]}, with glob + directory-prefix
    // matching. The 5.6 check stays as a secondary defense-in-depth pass
    // for the case where a sibling's paths[] is incomplete.
    {
        let own_paths_for_deep_check: Vec<String> = claim_paths
            .split(',')
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .collect();
        let collisions = check_lease_path_collisions(
            &lock_dir,
            &args.gap_id,
            &session_id,
            &own_paths_for_deep_check,
        );
        ...
        if !collisions.is_empty() {
            for c in &collisions {
                ...
                eprintln!(
                    "[claim] INFRA-1604: LEASE PATH COLLISION vs sibling session {} (gap {}) — {} overlapping path(s): {}",
                    c.sibling_session,
                    c.sibling_gap,
                    c.overlap_paths.len(),
...
// ── INFRA-1604: deep lease path-set collision detection ─────────────────────
//
// INFRA-1394 (above) only catches collisions when the gap's AC *text*
// mentions one of 5 hardcoded hot files. Real collisions happen on dozens
// of files that are declared in a lease's paths[] but never mentioned in
// AC text. This check computes the actual set intersection of {this
// claim's declared paths} ∩ {each sibling lease's declared paths}, with
// glob (`*`) and directory-prefix (trailing `/`) matching. Structural,
// not heuristic — the INFRA-1394 check stays as a secondary defense-in-depth
// pass for the case where a lease's paths[] is incomplete.
...
/// Simple glob matcher supporting `*` (matches any run of characters,
/// including `/`). No `?` or character-class support — lease path globs
/// in practice are things like `src/foo/*.rs`.
...
/// Do two lease-declared path entries overlap? Handles:
///   - exact match: `"src/foo.rs" == "src/foo.rs"`
///   - directory prefix: `"docs/"` overlaps `"docs/gaps/X.yaml"`
///   - glob: `"src/foo/*.rs"` overlaps `"src/foo/bar.rs"`
fn lease_paths_overlap(a: &str, b: &str) -> bool {
    let a = a.trim();
    let b = b.trim();
...
/// INFRA-1604: For each sibling lease under `lock_dir`, compute the set
/// intersection of `own_paths` against the sibling's declared `paths[]`.
/// Returns one `LeasePathCollision` per sibling with a non-empty overlap.
/// Best-effort: malformed/unreadable lease files are silently skipped —
/// this check must never block the claim flow on IO/parse errors.
    ...
    for entry in entries.flatten() {
        ...
        for op in own_paths {
            for sp in &sibling_paths {
                if lease_paths_overlap(op, sp) {
                    let entry = format!("{} <-> {}", op, sp);
                    if !overlap.contains(&entry) {
                        overlap.push(entry);
                    }
                }
            }
...
/// — never blocks the claim flow. Fired on every detection, whether it
/// results in a hard stop or a `--force-overlap` bypass.
