...
impl GapStore {
    ...
    fn migrate(&self) -> Result<()> {
        ...
        // INFRA-1551: drop dead schema — tables were schema'd but never written to
        // in production. read_live_intents() reads ambient.jsonl instead of the intents
        // table; routing_outcomes had no callers in the main chump binary (all inserts
        // lived in the separate chump-orchestrator binary which is not fleet-deployed).
        // Reversal: re-add CREATE TABLE blocks here with COG-036/intents comments and
        // remove these DROPs.
        let _ = self
            .conn
            .execute_batch("DROP TABLE IF EXISTS intents; DROP TABLE IF EXISTS routing_outcomes;");
...
- id: {}\n  domain: INFRA\n  title: superseded gap\n  status: done\n  closed_pr: 700\n",
    ...
    // (COG-036 routing-outcome tests removed in INFRA-1551 — table dropped)
