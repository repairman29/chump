...
impl GapStore {
    ...
    fn migrate(&self) -> Result<()> {
        ...
        // INFRA-1551: cull dead schema from existing DBs. routing_outcomes was
        // never populated in production (COG-036/037 code removed); intents was
        // never written (atomic_claim.rs reads ambient.jsonl instead). Safe to
        // drop unconditionally — IF EXISTS guards make this idempotent.
        let _ = self
            .conn
            .execute_batch("DROP TABLE IF EXISTS routing_outcomes; DROP TABLE IF EXISTS intents;");
