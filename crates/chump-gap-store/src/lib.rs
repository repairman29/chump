...
// routing_outcomes and intents tables removed (INFRA-1551): never populated
// in production. Thompson sampler (load_scoreboard_signatures in
// chump-orchestrator) gracefully falls back to YAML-only ordering when the
// table is absent. Reversal SQL in schema_migration.sql.
