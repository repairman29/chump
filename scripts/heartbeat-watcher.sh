...
#
# Usage:
#   scripts/heartbeat-watcher.sh start    # start daemon in background
#   scripts/heartbeat-watcher.sh stop     # stop running daemon
#   scripts/heartbeat-watcher.sh status   # show PID and daemon state
#
...
emit_event() {
    local kind="$1"
    shift
    local emit_script="$REPO_ROOT/scripts/ambient-emit.sh"
