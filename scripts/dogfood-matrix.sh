...
#
# Usage:
#   ./scripts/dogfood-matrix.sh                   # run all scenarios
#   ./scripts/dogfood-matrix.sh --quick           # fastest 3 (smoke)
#   ./scripts/dogfood-matrix.sh --scenario=read-large
#   ./scripts/dogfood-matrix.sh --list            # list scenarios & exit
#
# Output:
...
# scripts/restart-vllm-if-down.sh into their scheduler.
...
if ! curl -fsS --max-time 5 "$MODEL_BASE/models" >/dev/null 2>&1; then
  ...
  log "  Try: ./serve-vllm-mlx.sh  (or scripts/restart-vllm-if-down.sh)"
  exit 2
fi
