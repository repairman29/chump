...
#   bash scripts/screen-ocr.sh [IMAGE_PATH]
...
if [[ -z "$IMG" ]]; then
  ...
  if ( unset LD_LIBRARY_PATH; /system/bin/screencap -p "$IMG" 2>/dev/null ); then
    ...
    echo "screen-ocr: no image path given and screencap failed (need root or Termux:API). Use: bash scripts/screen-ocr.sh /path/to/image.png" >&2
    exit 1
  fi
  trap "rm -f '$IMG'" EXIT
fi
