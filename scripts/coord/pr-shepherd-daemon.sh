...
# Args: $1=pr_number $2=action $3=reason $4=gap_id [extra key=value pairs...]
...
# _emit_pr_action_taken_with_new_gap — variant that also carries new_gap_id
...
# Args: $1=job_name $2=signature (first 80 chars of error or job name)
...
# Pass FILED_GAPS_FILE as argv[2] to python3 so the shell variable expands correctly
# (avoid '$FILED_GAPS_FILE' literal inside python string which bypasses shell expansion).
_pr_already_filed_recently() {
  local fingerprint="$1"
  if [[ -z "$fingerprint" ]]; then return 1; fi
  if [[ ! -f "$FILED_GAPS_FILE" ]]; then return 1; fi
  # Check if fingerprint appears in the last 24h
...
def first_failure_info(checks):
    '''Return (job_name, signature) from the first FAILURE check.'''
    for ch in checks:
        conclusion = (ch.get('conclusion') or '').upper()
        if conclusion == 'FAILURE':
            job_name = ch.get('name') or ch.get('context') or 'unknown-job'
            details_url = ch.get('detailsUrl') or ch.get('targetUrl') or ''
            # Use first 80 chars of detailsUrl as signature fallback
            sig = details_url[:80] if details_url else job_name
            return job_name, sig
    return 'unknown-job', ''
...
for p in prs:
    ...
    # Extract failure info for BLOCKED_REAL_FAIL
    fail_job = ''
    fail_sig = ''
    if c == 'BLOCKED_REAL_FAIL':
        fail_job, fail_sig = first_failure_info(checks)
...
" 2>/dev/null || true)
  ...
  if [ -n "$classified" ]; then
    while IFS= read -r line; do
      ...
      # META-186: BLOCKED_GREEN → arm auto-merge
        ...
        # Execute arm auto-merge (idempotent — gh pr merge --auto is a no-op if already armed)
        if [ -n "$DRY_RUN" ]; then
      ...
      # META-186: BLOCKED_REAL_FAIL → file follow-up gap with dedup
        ...
        # Guard: dedup — already filed in last 24h?
        ...
        # Build gap title
        local short_job sig_short gap_title
        short_job="${fail_job:0:40}"
        sig_short="${fail_sig:0:60}"
        if [ -n "$sig_short" ] && [ "$sig_short" != "$short_job" ]; then
          gap_title="PR #${pr_num} failing on ${short_job}: ${sig_short}"
        else
          gap_title="PR #${pr_num} failing on ${short_job}"
        fi
        # Trim to 120 chars total
        ...
        if [ -n "$DRY_RUN" ]; then
          ...
          gap_out=$(chump gap reserve --domain INFRA \
            ...
            --effort xs \
            --force 2>&1) || true
          # Extract gap ID from output (format: "Reserved INFRA-NNNN" or similar)
...
" 2>/dev/null || echo "")
          if [ -n "$new_gap_id" ]; then
            ...
            echo "[pr-shepherd-daemon] gap reserve output: ${gap_out}" >&2
            echo "[pr-shepherd-daemon] WARN: gap reserve succeeded but no ID parsed — treating as filed" >&2
