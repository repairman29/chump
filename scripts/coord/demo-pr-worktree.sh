...
#
# Usage:
#   ./scripts/coord/demo-pr-worktree.sh                  # interactive (prompts before each step)
#   DEMO_AUTO=1 ./scripts/coord/demo-pr-worktree.sh      # non-interactive (runs all steps)
#
# Prerequisites:
...
PROMPT="Add a comment at the top of scripts/coord/demo-pr-worktree.sh that says '# Demo run at $(date -u +%Y-%m-%dT%H:%M:%SZ)'. Only add the one comment line, nothing else. Use the file at ${WORKTREE}/scripts/coord/demo-pr-worktree.sh."
...
if git diff --quiet && git diff --cached --quiet; then
  ...
  ls -la "$WORKTREE/scripts/coord/demo-pr-worktree.sh" 2>/dev/null || true
else
