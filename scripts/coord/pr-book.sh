...
# Calibration: ~/.chump/pr-book-calibration.log     (append-only Brier history)
#
# Env (test/override):
...
if [[ "$MODE" == "settle" ]]; then
  ...
  [[ -z "$OMAP" ]] && OMAP='{}'
  # Brier over every resolved prediction row (integer math done in jq, not bash).
  RES="$(jq -n --slurpfile L "$LEDGER" --argjson O "$OMAP" '
    [ $L[] | (.pr|tostring) as $k | select($O[$k] != null)
             | (.price - $O[$k]) as $e | ($e*$e) ] as $sq
    | { resolved: ($sq|length),
        merged:   ([ $L[] | select($O[(.pr|tostring)]==1) ] | length),
        closed:   ([ $L[] | select($O[(.pr|tostring)]==0) ] | length),
        predictions: ($L|length),
        brier: (if ($sq|length)>0 then (((($sq|add)/($sq|length))*10000)|round)/10000 else null end) }')"
  echo "$RES" | jq -c --arg ts "$TS" '{ts:$ts} + .' >> "$CALIB"
  ...
  printf "  resolved %s of %s predictions (%s merged / %s closed)\n" \
    "$RESOLVED" "$(echo "$RES"|jq -r '.predictions')" "$MERGED" "$CLOSED"
