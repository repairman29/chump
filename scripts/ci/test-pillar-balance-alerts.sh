...
#   2.  Bash 3.2 shebang (no /usr/local/bin/bash)
...
reserve_gap() {
    # reserve_gap <domain> <priority> <effort> <title> [--ac <text>]
...
# Add 2+ pickable gaps per pillar with AC
...
# ZERO-WASTE needs its own domain; use INFRA with title prefix
...
# Only fill 3 pillars — ZERO-WASTE gets 0
for domain in EFFECTIVE CREDIBLE RESILIENT; do
    reserve_gap "$domain" P1 xs "${domain}: gap-one" "AC: ships" > /dev/null
    reserve_gap "$domain" P1 xs "${domain}: gap-two" "AC: ships" > /dev/null
done
...
# Put 3 EFFECTIVE gaps and 1 CREDIBLE → EFFECTIVE = 75% of pool (>50%)
...
# Only fill EFFECTIVE (< 2 for others → would emit alerts if not dry run)
reserve_gap EFFECTIVE P1 xs "EFFECTIVE: gap-one" "AC: ships" > /dev/null
...
# Add gaps WITH AC for EFFECTIVE only; leave CREDIBLE with vague-AC (no AC)
reserve_gap EFFECTIVE P1 xs "EFFECTIVE: gap-one" "AC: ships" > /dev/null
reserve_gap EFFECTIVE P1 xs "EFFECTIVE: gap-two" "AC: ships" > /dev/null
# CREDIBLE gap with NO acceptance_criteria → vague, excluded
