...
#
# Targets:
#   - chump --version warm-cache: < 500ms wall time
#   - chump --help    warm-cache: < 500ms wall time
...
# Find a chump binary. Prefer release; fall back to debug in the resolved
# target dir (respects CARGO_TARGET_DIR, e.g. shared-target worktrees).
CHUMP="$TARGET_DIR/release/chump"
if [[ ! -x "$CHUMP" ]]; then CHUMP="$TARGET_DIR/debug/chump"; fi
if [[ ! -x "$CHUMP" ]]; then
...
# ── Test 4: tight CHUMP_STARTUP_TIMEOUT_MS doesn't trip --version path ───────
