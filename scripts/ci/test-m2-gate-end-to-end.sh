...
for i in 1 2 3 4; do
    bash "$L6A_SUPERVISOR" record TEST-M2-GAP 2>/dev/null
done
...
for i in 1 2 3 4; do
    bash "$L6A_SUPERVISOR" record TEST-M2-GAP-2 2>/dev/null
done
...
if ! bash "$L6C_GUARDRAIL" TEST-M2 "src/main.rs,docs/test.md" 2>/dev/null; then
...
if bash "$L6C_GUARDRAIL" TEST-M2 "docs/test.md" 2>/dev/null; then
