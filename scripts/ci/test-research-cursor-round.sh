...
1. PICK A GOAL: read_file docs/strategy/ROADMAP.md and read_file docs/briefs/CHUMP_PROJECT_BRIEF.md. Pick from an unchecked roadmap item, an open task, or Chump–Cursor improvements. Do not invent your own roadmap—use the files. Use web_search if it helps (1–2 queries); store key findings in memory.
...
3. USE CURSOR TO IMPLEMENT: run_cli with agent --model auto -p "<clear goal from roadmap or task; include that Cursor should read docs/strategy/ROADMAP.md and docs/briefs/CHUMP_PROJECT_BRIEF.md>" --force. Goal is real product improvement.

4. WRAP UP: episode log; update ego; set task status if relevant. If you completed a roadmap item, patch_file or write_file docs/strategy/ROADMAP.md to change - [ ] to - [x]. notify if something is ready. Be concise.'
