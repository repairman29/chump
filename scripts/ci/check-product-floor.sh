...
#
# Usage:
#   ./scripts/ci/check-product-floor.sh              # human output + ALERT on drought
#   ./scripts/ci/check-product-floor.sh --json       # json summary to stdout
#   ./scripts/ci/check-product-floor.sh --days 14    # custom window (default 7)
#   ./scripts/ci/check-product-floor.sh --quiet      # suppress output when healthy
#
# Exit codes:
...
if [[ $DROUGHT == "1" ]]; then
    if [[ -x "$REPO_ROOT/scripts/coord/broadcast.sh" ]]; then
        "$REPO_ROOT/scripts/coord/broadcast.sh" ALERT kind=product_drought \
