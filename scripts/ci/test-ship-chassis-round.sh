...
#
# Usage: ./scripts/ci/test-ship-chassis-round.sh
...
if command -v timeout >/dev/null 2>&1; then
  timeout "${TIMEOUT_SEC}s" env HEARTBEAT_ONE_ROUND=1 CHUMP_HOME="$ROOT" CHUMP_SHIP_TARGET=chump-chassis ./scripts/dev/heartbeat-ship.sh >> "$SHIP_LOG" 2>&1 || true
elif command -v gtimeout >/dev/null 2>&1; then
  gtimeout "${TIMEOUT_SEC}s" env HEARTBEAT_ONE_ROUND=1 CHUMP_HOME="$ROOT" CHUMP_SHIP_TARGET=chump-chassis ./scripts/dev/heartbeat-ship.sh >> "$SHIP_LOG" 2>&1 || true
else
  env HEARTBEAT_ONE_ROUND=1 CHUMP_HOME="$ROOT" CHUMP_SHIP_TARGET=chump-chassis ./scripts/dev/heartbeat-ship.sh >> "$SHIP_LOG" 2>&1 || true
fi
RUN_END=$(date +%s)
