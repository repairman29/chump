...
#
# Usage:
#   ./scripts/ci/battle-qa.sh                    # Run once, report
#   BATTLE_QA_ITERATIONS=5 ./scripts/ci/battle-qa.sh   # Re-run up to 5 times until all pass or max
#   BATTLE_QA_TIMEOUT=60 ./scripts/ci/battle-qa.sh    # 60s per query (default 90)
#   BATTLE_QA_SKIP=100 ./scripts/ci/battle-qa.sh      # Skip first 100 queries (resume)
#   BATTLE_QA_MAX=50 ./scripts/ci/battle-qa.sh        # Run only first 50 (smoke test)
#
...
# Tail alongside web dogfood: ./scripts/eval/tail-model-dogfood.sh (see docs/operations/MODEL_TESTING_TAIL.md)
...
#
# EVAL-003: --with-replay runs scripts/eval/replay-trajectory.sh against the saved
...
# Preflight
if ! port=$(./scripts/ci/check-heartbeat-preflight.sh 2>/dev/null); then
...
while [[ $iteration -le $ITERATIONS ]]; do
  ...
  if run_suite "iter-$iteration"; then
    ...
    if [[ -x "$ROOT/scripts/eval/consciousness-baseline.sh" ]]; then
      echo "" | tee -a "$LOG"
      echo "=== Consciousness Metrics (post-battle-QA) ===" | tee -a "$LOG"
      "$ROOT/scripts/eval/consciousness-baseline.sh" 2>&1 | tail -8 | tee -a "$LOG"
...
# the live-driver acceptance clause "scripts/ci/battle-qa.sh wires in the replay
...
if [[ $WITH_REPLAY -eq 1 ]]; then
  ...
  REPLAY_SCRIPT="$ROOT/scripts/eval/replay-trajectory.sh"
