...
ERROR_OUT=$("$BIN" claim INFRA-XXXX --role test --check-only 2>&1 || true)
...
EXPIRED_OUT=$("$BIN" claim INFRA-YYYY --role test --check-only 2>&1 || true)
...
NO_LEASE_OUT=$("$BIN" claim INFRA-ZZZZ --role test --check-only 2>&1 || true)
