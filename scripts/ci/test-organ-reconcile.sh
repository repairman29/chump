...
# ── 10. INFRA-7328: --check parses node-organ-manifest.txt (non-systemd
...
pass "10a: --check reports DETECTED-ALIVE (pgrep + heartbeat fallback) and DEAD for node-organ-manifest.txt, non-zero exit on any DEAD"
...
pass "10b: --check exits 0 when every required process-organ is DETECTED-ALIVE"
