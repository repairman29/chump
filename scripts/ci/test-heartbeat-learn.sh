...
if command -v timeout >/dev/null 2>&1; then
  timeout "$HEARTBEAT_TEST_TIMEOUT" env HEARTBEAT_DURATION=1m HEARTBEAT_INTERVAL=20s ./scripts/dev/heartbeat-learn.sh || EXIT=$?
else
  HEARTBEAT_DURATION=1m HEARTBEAT_INTERVAL=20s ./scripts/dev/heartbeat-learn.sh || EXIT=$?
fi
