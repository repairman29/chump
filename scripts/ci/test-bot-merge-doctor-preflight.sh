#!/usr/bin/env bash
# INFRA-422 — verify bot-merge.sh runs chump-doctor.sh as preflight before
...
DOC="$REPO_ROOT/scripts/dev/chump-doctor.sh"

[[ -x "$BM" ]]  || { fail "bot-merge.sh missing or not executable"; exit 1; }
[[ -x "$DOC" ]] || { fail "chump-doctor.sh missing or not executable"; exit 1; }
pass "bot-merge.sh and chump-doctor.sh both exist + executable"

# 1. bot-merge.sh references chump-doctor.sh.
grep -q 'chump-doctor.sh' "$BM" \
    && pass "bot-merge.sh references chump-doctor.sh" \
    || fail "bot-merge.sh has no chump-doctor.sh invocation"
...
DOCTOR_LINE=$(grep -n 'scripts/dev/chump-doctor.sh' "$BM" | head -1 | cut -d: -f1 || echo 0)
...
# 4. chump-doctor.sh itself probes-then-heals (the contract bot-merge depends on).
grep -q 'gtimeout.*--version' "$DOC" \
    && pass "chump-doctor.sh probes via --version + gtimeout" \
    || fail "chump-doctor.sh missing the --version probe"

grep -q 'wedged-inode' "$DOC" \
    && pass "chump-doctor.sh has the inode-replacement heal path" \
    || fail "chump-doctor.sh missing the heal path"
