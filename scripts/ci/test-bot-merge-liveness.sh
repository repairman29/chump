...
test -f scripts/coord/bot-merge-run-timed.py || {
    echo "missing scripts/coord/bot-merge-run-timed.py" >&2
    exit 1
}

grep -q "run_timed" scripts/coord/bot-merge.sh || {
    echo "bot-merge.sh missing run_timed" >&2
    exit 1
}
grep -q "heartbeat_begin" scripts/coord/bot-merge.sh || {
    echo "bot-merge.sh missing heartbeat_begin" >&2
    exit 1
}
grep -q "No GitHub PR found for branch" scripts/coord/bot-merge.sh || {
...
set +e
out=$(python3 scripts/coord/bot-merge-run-timed.py 1 -- sleep 60 2>&1)
rc=$?
set -e
if [[ "$rc" -ne 124 ]]; then
