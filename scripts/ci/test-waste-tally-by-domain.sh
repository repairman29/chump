...
if grep -qE 'pub struct WasteDomainReport' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
    ok "pub struct WasteDomainReport exists"
else
    fail "WasteDomainReport missing or not pub"
fi

if grep -qE 'pub fn build_domain_report\b' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
    ok "pub fn build_domain_report exists"
else
    fail "build_domain_report missing or not pub"
fi

if grep -qE 'pub struct WasteDomainEntry' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
...
if grep -qE 'fn domain_from_gap_id' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
...
if grep -qE 'impl WasteDomainReport' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
    ok "impl WasteDomainReport block exists"
else
    fail "impl WasteDomainReport missing"
fi

if grep -q 'fn render_text' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs" && \
   grep -q 'fn render_json' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
...
if grep -q 'Reverse(e.incidents)' "$REPO_ROOT/crates/chump-waste-tally/src/waste_tally.rs"; then
