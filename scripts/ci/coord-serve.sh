...
#
# Usage:
#   scripts/ci/coord-serve.sh          # start NATS (idempotent — no-op if running)
#   scripts/ci/coord-serve.sh --stop   # stop NATS
#   scripts/ci/coord-serve.sh --status # show status
#   scripts/ci/coord-serve.sh --init   # init buckets/streams only (NATS already running)
#
# Install nats-server:
...
if server_running; then
    ...
    if [[ -n "$COORD_BIN" ]]; then
        ...
        "$COORD_BIN" ping >/dev/null 2>&1 && \
            ...
            printf '%s chump-coord ping failed — run: scripts/ci/coord-serve.sh --init\n' "$(dim '!')"
    else
