...
if [[ -n "$PIXEL_HOST" ]]; then
  if ssh -o ConnectTimeout=15 -o StrictHostKeyChecking=no -p "$PIXEL_PORT" "$PIXEL_HOST" 'cd ~/chump && bash scripts/restart-mabel-heartbeat.sh' 2>/dev/null; then
...
if bash "$ROOT/scripts/restart-chump-heartbeat.sh" 2>/dev/null; then
