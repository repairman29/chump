...
# add them to scripts/chump-mode.conf to give more headroom to vLLM-MLX / Python.
# Run before "Enter Chump mode" to see what to close.
#
# Usage: ./scripts/list-heavy-processes.sh [N]
...
echo "" | tee -a "$LOG"
echo "Log written to $LOG — uncomment matching names in scripts/chump-mode.conf then run Enter Chump mode." | tee -a "$LOG"
