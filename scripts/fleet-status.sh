...
echo "---------- musher ----------"
MUSHER_TEXT=""
if [[ -x scripts/musher.sh ]] || [[ -f scripts/musher.sh ]]; then
  MUSHER_TEXT=$(bash scripts/musher.sh --status 2>&1) || echo "[fleet-status] WARN: musher --status exited non-zero"
  echo "$MUSHER_TEXT"
else
  echo "[fleet-status] WARN: scripts/musher.sh not found"
fi
echo ""
