...
#
# Usage:
#   ./scripts/deploy-fleet.sh              # full fleet deploy
#   ./scripts/deploy-fleet.sh --mac        # Mac only (skip Pixel)
#   ./scripts/deploy-fleet.sh --pixel      # Pixel only (skip Mac; reuses existing Android binary if fresh)
#   ./scripts/deploy-fleet.sh --no-build   # skip both builds (re-deploy existing binaries)
#   ./scripts/deploy-fleet.sh --health     # health check only; no deploy
#
# Env overrides:
