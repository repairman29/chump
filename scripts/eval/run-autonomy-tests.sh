...
# From repo root: ./scripts/eval/run-autonomy-tests.sh
...
run_chump() {
  ...
  if [[ "${OPENAI_API_BASE:-}" == *":8000"* ]] && echo "$out" | grep -qE "connection closed|Connection refused|error sending request"; then
    [[ -x "$ROOT/scripts/setup/restart-vllm-if-down.sh" ]] && "$ROOT/scripts/setup/restart-vllm-if-down.sh" >/dev/null 2>&1 || true
    sleep 15
    if [[ -x "$ROOT/target/release/chump" ]]; then
      out=$(run_with_timeout "$AUTONOMY_TIMEOUT" "${CHUMP_CMD[@]}" "$prompt" 2>&1)
    else
...
ensure_model_up() {
  if [[ "${OPENAI_API_BASE:-}" == *":8000"* ]] && [[ -x "$ROOT/scripts/setup/restart-vllm-if-down.sh" ]]; then
    "$ROOT/scripts/setup/restart-vllm-if-down.sh" >/dev/null 2>&1 || true
  fi
}
...
if [[ -z "${TAVILY_API_KEY:-}" ]] || [[ "${TAVILY_API_KEY}" == "your-tavily-api-key" ]]; then
  ...
  ./scripts/ci/test-heartbeat-learn.sh 2>&1 | tee -a "$ROOT/logs/autonomy-tier4.log"; tier4_exit=${PIPESTATUS[0]}
  if [[ $tier4_exit -eq 0 ]]; then
    # Verify a model provider is still reachable (local or cascade) after the heartbeat round.
    if post_port=$(./scripts/ci/check-heartbeat-preflight.sh 2>/dev/null); then
...
if [[ -z "${CHUMP_REPO:-}" ]] && [[ -z "${CHUMP_HOME:-}" ]]; then
  ...
  if [[ -x "$ROOT/scripts/ci/test-tier5-self-improve.sh" ]]; then
    if "$ROOT/scripts/ci/test-tier5-self-improve.sh" 2>&1 | tee -a "$ROOT/logs/autonomy-tier5.log"; then
    ...
    echo "SKIP (scripts/ci/test-tier5-self-improve.sh not found)"
  fi
fi
