...
#
# Usage:
#   ./scripts/eval/run-consciousness-study.sh                   # full study
#   CHUMP_STUDY_SKIP_BUILD=1 ./scripts/eval/run-consciousness-study.sh  # skip build
#
# Outputs:
...
run_battery() {
  ...
  # Capture baseline
  bash "$ROOT/scripts/eval/consciousness-baseline.sh" > /dev/null 2>&1
  cp "$LOG_DIR/consciousness-baseline.json" "$LOG_DIR/study-${condition}-baseline.json"

  # Capture report
  bash "$ROOT/scripts/eval/consciousness-report.sh" > "$LOG_DIR/study-${condition}-report.txt" 2>&1 || true
...
bash "$ROOT/scripts/eval/analyze-ab-results.sh" 2>/dev/null || echo "  (analysis script not found, skipping)"

# ─── Phase 7: Generate draft ──────────────────────────────────────

echo "Phase 7: Generating draft report..."
bash "$ROOT/scripts/eval/generate-research-draft.sh" 2>/dev/null || echo "  (draft generator not found, skipping)"
