...
#
# Usage: ./scripts/eval/generate-sprint-synthesis.sh [CHUMP_HOME]
...
if [[ -x "$ROOT/scripts/coord/harvest-synthesis-lessons.sh" ]]; then
  "$ROOT/scripts/coord/harvest-synthesis-lessons.sh" "$OUT" "$ROOT" || true
fi
