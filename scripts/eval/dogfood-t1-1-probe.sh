...
#
# Usage:
#   OPENAI_MODEL=qwen2.5:14b ./scripts/eval/dogfood-t1-1-probe.sh
#   OPENAI_MODEL=qwen3:8b OPENAI_API_BASE=http://127.0.0.1:11434/v1 ./scripts/eval/dogfood-t1-1-probe.sh
#
# Logs:
...
if ! "$ROOT/scripts/ci/check-heartbeat-preflight.sh" &>/dev/null; then
  echo "error: model server preflight failed. Start Ollama, or for vLLM on 8000 run ./scripts/setup/restart-vllm-if-down.sh then ./scripts/setup/wait-for-vllm.sh (poll only; avoids overlapping starts during HF download)." >&2
  exit 1
fi
...
"$ROOT/scripts/eval/dogfood-run.sh" "$PROMPT"
DOG_EXIT=$?
