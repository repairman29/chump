...
#   MABEL_HEARTBEAT_QUICK_TEST=1 ./scripts/dev/heartbeat-mabel.sh   # 2m duration, 30s interval
#
...
2. HEALTH CHECK: run_cli to check local llama-server: curl -s localhost:${LOCAL_PORT}/v1/models | head -1.
   ...
   If anything is down, try fix via SSH: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'cd ${MAC_HOME} && bash scripts/dev/farmer-brown.sh'\"
...
3. CHECK CHUMP HEARTBEAT: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'tail -5 ${MAC_HOME}/logs/heartbeat-self-improve.log'\"
   If the last round was >30 min ago or shows repeated failures: restart via script: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'cd ${MAC_HOME} && bash scripts/setup/restart-chump-heartbeat.sh'\"
...
3.5. CHECK SHIP HEARTBEAT: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'tail -8 ${MAC_HOME}/logs/heartbeat-ship.log'\"
   If the log is missing, or last line is older than 15 min, or shows no recent Round: ensure ship is running: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'cd ${MAC_HOME} && bash scripts/setup/ensure-ship-heartbeat.sh'\"
...
0. PEER APPROVAL (two-key): If memory_brain read_file a2a/pending_approval.json exists and contains request_id and tool_name (e.g. git_push, merge_pr), Chump is waiting for your approval. Run tests on the Mac: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'cd ${MAC_HOME} && cargo test 2>&1 | tail -20'\". If tests pass and the request looks safe, approve via run_cli: curl -s -X POST http://${MAC_IP}:${MAC_WEB_PORT:-3000}/api/approve -H \"Authorization: Bearer \$CHUMP_WEB_TOKEN\" -H \"Content-Type: application/json\" -d '{\"request_id\":\"<request_id from file>\",\"allowed\":true}' (replace <request_id from file> with the actual request_id from the pending_approval file). If tests fail or you are unsure, use \"allowed\":false. Then proceed to step 1.
1. Check Chump's last episode: run_cli \"ssh -o StrictHostKeyChecking=no -p ${MAC_SSH_PORT} ${MAC_USER}@${MAC_IP} 'cd ${MAC_HOME} && sqlite3 sessions/chump_memory.db \"SELECT summary, detail FROM chump_episodes ORDER BY created_at DESC LIMIT 1\"'\"
   If the last episode was a code change (commit, PR, patch_file, cargo, git): proceed to step 2. Otherwise skip to step 5.
...
1. DIAGNOSE: run_cli \"cd $ROOT && MABEL_FARMER_DIAGNOSE_ONLY=1 bash scripts/dev/mabel-farmer.sh 2>&1 | tail -40\"
...
4. ACT: If diagnose shows need_fix, run fix: run_cli \"cd $ROOT && MABEL_FARMER_DIAGNOSE_ONLY=0 bash scripts/dev/mabel-farmer.sh 2>&1 | tail -20\". If still bad after fix, notify with a short actionable message (reference logs/mabel-farmer.log). Otherwise use message_peer only when Chump should self-repair (e.g. remote fix failed).
...
while true; do
  ...
  case "$round_type" in
    patrol)
      ...
      if [[ -x "$ROOT/scripts/dev/mabel-farmer.sh" ]]; then
        echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Round $round (patrol): running mabel-farmer.sh" >> "$LOG"
        MABEL_FARMER_DIAGNOSE_ONLY=0 "$ROOT/scripts/dev/mabel-farmer.sh" >> "$LOG" 2>&1 || true
      fi
