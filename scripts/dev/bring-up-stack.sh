...
#
# Usage:
#   ./scripts/dev/bring-up-stack.sh                    # start stack (no pull/build)
#   PULL=1 ./scripts/dev/bring-up-stack.sh             # git pull, then build and start
#   BUILD_ONLY=1 ./scripts/dev/bring-up-stack.sh        # only build release; don't start processes
#
...
if [[ "$HEARTBEATS" == "1" ]]; then
  ...
  if [[ "${OPENAI_API_BASE:-}" == *":8000"* ]] && [[ -f "$ROOT/scripts/dev/env-max_m4.sh" ]]; then
    source "$ROOT/scripts/dev/env-max_m4.sh"
