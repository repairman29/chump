...
#
# Usage:
#   scripts/dev/start-ambient-watch.sh           # start in background
#   scripts/dev/start-ambient-watch.sh --stop    # stop the running daemon
...
# Start daemon
"$REPO_ROOT/scripts/dev/ambient-watch.sh" >> "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
