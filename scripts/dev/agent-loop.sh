...
#
# Usage:
#   scripts/dev/agent-loop.sh                   # loop forever
#   scripts/dev/agent-loop.sh --max-gaps 5      # stop after 5 gaps
#   scripts/dev/agent-loop.sh --interval 90     # seconds between runs (default: 60)
...
Pick ONE gap, do the work, ship it via scripts/coord/bot-merge.sh --gap <ID> --auto-merge, \
then exit. Do not call ScheduleWakeup — the shell loop handles retries."

check_queue() {
    python3.12 scripts/coord/musher.py --pick >/dev/null 2>&1
}
