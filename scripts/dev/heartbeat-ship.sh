...
#
# Usage:
#   ./scripts/dev/heartbeat-ship.sh                          # 8h, 5m rounds (cascade) or 45m (local)
#   HEARTBEAT_DURATION=24h ./scripts/dev/heartbeat-ship.sh   # 24h autonomy (one run then exit; restart via ensure-ship-heartbeat or cron)
#   HEARTBEAT_DURATION=4h HEARTBEAT_INTERVAL=10m ./scripts/dev/heartbeat-ship.sh
#   HEARTBEAT_QUICK_TEST=1 ./scripts/dev/heartbeat-ship.sh   # 2 min, 30s rounds
#   HEARTBEAT_DRY_RUN=1 ./scripts/dev/heartbeat-ship.sh      # No push/PR, log only
#   HEARTBEAT_ONE_ROUND=1 ./scripts/dev/heartbeat-ship.sh     # One ship round then exit (no wait)
#   CHUMP_AUTOPILOT=1 ./scripts/dev/heartbeat-ship.sh        # Short sleep between rounds (default 5s). More rounds = more API use.
#   HEARTBEAT_STRICT_LOG=1 ./scripts/dev/heartbeat-ship.sh   # Warn when ship round ok but no project log updated.
#   HEARTBEAT_DEBUG=1 ./scripts/dev/heartbeat-ship.sh        # Write last 80 lines of each round to logs/heartbeat-ship-round-N.log.
#
...
# If you see "Preflight FAIL: no model reachable" in the log, run ./scripts/ci/check-heartbeat-preflight.sh
# and ./scripts/ci/check-providers.sh from the same shell after sourcing .env; ensure cascade keys/scopes (e.g. GitHub models:read) are valid.
...
if [[ -n "$PIXEL_HOST" ]]; then
  MABEL_CHECK="0. CHECK MABEL: run_cli \"ssh -o StrictHostKeyChecking=no -o ConnectTimeout=15 -p ${PIXEL_PORT} ${PIXEL_HOST} 'tail -5 ~/chump/logs/heartbeat-mabel.log'\"
   If last log >30 min old or repeated failures: run_cli \"ssh -p ${PIXEL_PORT} ${PIXEL_HOST} 'cd ~/chump && bash scripts/setup/restart-mabel-heartbeat.sh'\"
...
port=""
if port=$("$ROOT/scripts/ci/check-heartbeat-preflight.sh" 2>/dev/null); then
  : # preflight ok
  ...
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Preflight FAIL: no model reachable. Run ./scripts/ci/check-providers.sh to verify cascade keys (401 = missing models scope, e.g. GitHub needs models:read)." >> "$LOG"
  echo "Preflight FAIL: no model reachable. Run ./scripts/ci/check-providers.sh to verify keys." >&2
  exit 1
fi
[[ "$CASCADE_ON" -eq 1 ]] && ./scripts/ci/check-providers.sh 2>&1 | head -30 >> "$LOG" || true
