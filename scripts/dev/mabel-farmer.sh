...
#
# Usage:
#   ./scripts/dev/mabel-farmer.sh                          # diagnose + fix once
#   MABEL_FARMER_DIAGNOSE_ONLY=1 ./scripts/dev/mabel-farmer.sh  # diagnose only
#   MABEL_FARMER_INTERVAL=120 ./scripts/dev/mabel-farmer.sh     # loop every 120s
#
...
# ─────────────────────────────────────────────────────────────
run_remote_fix() {
  local fix_cmd="${MABEL_FARMER_FIX_CMD:-cd $MAC_HOME && ./scripts/dev/farmer-brown.sh}"
...
run_ship_restart() {
  ...
  local output
  output=$($SSH_CMD "cd $MAC_HOME && bash scripts/setup/restart-ship-heartbeat.sh" 2>&1) || true
