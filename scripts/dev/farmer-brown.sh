...
#
# Usage:
#   ./scripts/dev/farmer-brown.sh           # diagnose + fix once
#   FARMER_BROWN_DIAGNOSE_ONLY=1 ./scripts/dev/farmer-brown.sh   # diagnose only, no fix
#   FARMER_BROWN_INTERVAL=120 ./scripts/dev/farmer-brown.sh      # loop every 120s (diagnose + fix)
#
...
run_fix() {
  ...
  CHUMP_KEEPALIVE_INTERVAL= ./scripts/dev/keep-chump-online.sh
}
