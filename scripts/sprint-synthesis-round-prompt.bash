...
1. **Run the synthesis script:**
   ```
   run_cli "./scripts/generate-sprint-synthesis.sh"
   ```
...
4. **Commit and push:**
   ```
   run_cli "scripts/chump-commit.sh <synthesis-file-path> -m 'docs(synthesis): auto-generate $(date +%Y-%m-%d) session synthesis'"
   ```
...
- DRY_RUN → `run_cli "CHUMP_DRY_RUN=1 ./scripts/generate-sprint-synthesis.sh"` and print the context block only; do not write or commit.
