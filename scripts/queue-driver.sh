...
# in this repo). Calls scripts/resolve-gaps-conflict.py — refuses if any
# other file conflicts so non-append conflicts still get human attention.
#
# Usage:
#   scripts/queue-driver.sh                 # process oldest BEHIND/DIRTY, exit
#   scripts/queue-driver.sh --dry-run       # report what it would do
#   scripts/queue-driver.sh --max N         # process up to N PRs (default 1)
#
...
RESOLVER="$REPO_ROOT/scripts/resolve-gaps-conflict.py"
