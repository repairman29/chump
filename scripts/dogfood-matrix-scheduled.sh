...
#
# Usage:
#   ./scripts/dogfood-matrix-scheduled.sh               # full matrix
#   MODE=quick ./scripts/dogfood-matrix-scheduled.sh    # quick smoke only
#
# Env:
...
set +e
./scripts/dogfood-matrix.sh "${MATRIX_ARGS[@]}"
MATRIX_RC=$?
set -e
...
Re-run locally: ./scripts/dogfood-matrix.sh --scenario=<name>
EOF
)
