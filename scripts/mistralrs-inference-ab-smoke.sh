...
#
# Usage:
#   ./scripts/mistralrs-inference-ab-smoke.sh print          # print env recipes (modes A/B/C)
#   ./scripts/mistralrs-inference-ab-smoke.sh http           # AB-2 with OPENAI_API_BASE from .env
#   ./scripts/mistralrs-inference-ab-smoke.sh inproc         # AB-2 with mistral primary (unset OPENAI_API_BASE)
#
# Env:
...
=== Mode C: In-process mistral.rs ===
  ...
  ./scripts/mistralrs-inference-ab-smoke.sh inproc

Micro-bench (in-process only): ./scripts/bench-mistralrs-chump.sh --help
Battle QA smoke: BATTLE_QA_MAX=20 ./scripts/battle-qa.sh
EOF
}
