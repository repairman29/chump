...
is_todo_ac() {
    local ac="$1"
    echo "$ac" | grep -qiE '^\s*(TODO|To Do|fixme|FIXME)\s*$' 2>/dev/null || [[ "$ac" =~ ^[[:space:]]*(TODO|[Tt]o [Dd]o|fixme|FIXME)[[:space:]]*$ ]] || return 1
    return 0
}
...
while IFS= read -r gap; do
    ...
    case "$pillar" in
        EFFECTIVE)
            pillar_count_EFFECTIVE=$((pillar_count_EFFECTIVE + 1))
            ;;
        CREDIBLE)
            pillar_count_CREDIBLE=$((pillar_count_CREDIBLE + 1))
            ;;
        RESILIENT)
            pillar_count_RESILIENT=$((pillar_count_RESILIENT + 1))
            ;;
        ZERO-WASTE)
            pillar_count_ZERO_WASTE=$((pillar_count_ZERO_WASTE + 1))
            ;;
        OTHER)
            pillar_count_OTHER=$((pillar_count_OTHER + 1))
            ;;
    esac
...
# Helper to get count for a pillar (Bash 3.2 compatible)
get_pillar_count() {
    local pillar="$1"
    case "$pillar" in
