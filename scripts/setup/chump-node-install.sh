...
# role=common entries (INFRA-3650: organs every role needs regardless of
# brain vs muscle, e.g. process-organ-heal) are always included, mirroring
# the old common_organs()'s "always appended" semantics. Skip reasons go to
# stderr so `list="$(applicable_organs ...)"` stays clean.
applicable_organs() {
  local want_role="$1"
  ...
  while read -r state name rest; do
    ...
    role="${role:-brain}"
    [ -z "$ex" ] && ex="$name.sh"
    if [ "$role" != common ] && [ "$want_role" != all ] && [ "$role" != "$want_role" ]; then
      continue
    fi
    local reason=""
...
self_test() {
  ...
  # role=common entries (e.g. process-organ-heal, INFRA-3650) are always
  # included regardless of --role, same as the old common_organs() behavior.
  local list; list="$(applicable_organs "$ROLE")"
