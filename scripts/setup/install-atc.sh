#!/usr/bin/env bash
# scripts/setup/install-atc.sh — RESILIENT-300, auto-deploy path INFRA-3593
# (RESILIENT-1101: renamed from install-helsinki-atc.sh — helsinki was
# decommissioned 2026-08-17 and this logic is node-neutral via --role
# brain|muscle. A scripts/setup/install-helsinki-atc.sh compat symlink is
# kept so existing callers/docs keep working.)
#
# Establishes the full "ATC roster" on the brain-role primary node (owned
# iron) from a fresh clone: the self-maintenance daemons that keep the
# shipping pipeline moving without an agent having to remember them:
#
...
# on the node with no human step — mirroring how node-refresh already
...
#
# Usage:
#   sudo bash scripts/setup/install-atc.sh
#   sudo bash scripts/setup/install-atc.sh --check   # exit 0 iff all timers active
#   bash scripts/setup/install-atc.sh --auto         # merge-triggered auto-deploy
#   (install-helsinki-atc.sh also works — it's a compat symlink to this file)
