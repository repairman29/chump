...
# rust-main-append removed from the loop (INFRA-1526, 2026-05-23 P0 fix).
# src/main.rs was removed from .gitattributes because the append-only driver
# silently dropped non-pure-append hunks on conflict instead of emitting
# conflict markers. The driver registration is left in .git/config for
# existing checkouts that may have stale .gitattributes — it is harmless
# when not referenced by .gitattributes.
...
if [[ -f .gitattributes ]]; then
    ...
    # src/main.rs intentionally absent — removed from append-only driver (INFRA-1526).
