#!/usr/bin/env bash
# scripts/setup/install-daemon-activator.sh — META-225
#
# Idempotently installs the launchd agent that runs daemon-activator-loop.sh
# every 5 minutes (StartInterval: 300).
#
# Usage:
#   bash scripts/setup/install-daemon-activator.sh        # install + load
#   launchctl unload ~/Library/LaunchAgents/com.chump.daemon-activator.plist
#
...
LABEL="com.chump.daemon-activator"
PLIST="$HOME/Library/LaunchAgents/${LABEL}.plist"
BOT_SCRIPT="$ROOT/scripts/coord/daemon-activator-loop.sh"
LOG_OUT="$HOME/.chump/logs/daemon-activator.out"
LOG_ERR="$HOME/.chump/logs/daemon-activator.err"
...
<dict>
    ...
    <!-- RunAtLoad=true: exercises the daemon immediately on install so
         ...
         5 minutes later (INFRA-351 lesson). -->
    <key>RunAtLoad</key>
    <true/>
