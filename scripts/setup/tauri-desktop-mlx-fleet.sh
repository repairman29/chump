...
# Optional:
#   CHUMP_TAURI_FLEET_USE_MAX_M4=1  — source scripts/dev/env-max_m4.sh before live web check (strict 8000 profile).
...
if [[ "${CHUMP_TAURI_FLEET_USE_MAX_M4:-}" == "1" ]]; then
  # shellcheck source=scripts/dev/env-max_m4.sh
  source "$ROOT/scripts/dev/env-max_m4.sh"
  echo "[env] CHUMP_TAURI_FLEET_USE_MAX_M4=1 → sourced scripts/dev/env-max_m4.sh"
fi
