...
#
# Usage: bash scripts/setup/restart-mabel.sh [--force] [--dry-run]
...
if [[ "${FORCE}" == "1" ]]; then
  ...
  run "cd ~/chump && bash scripts/setup/ensure-mabel-bot-up.sh"
else
  echo "restart-mabel: calling ensure-mabel-bot-up.sh on Pixel..."
  if ! run "cd ~/chump && bash scripts/setup/ensure-mabel-bot-up.sh"; then
