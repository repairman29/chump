...
# Use this after `./scripts/restart-vllm-if-down.sh` or `./serve-vllm-mlx.sh` so health checks and
# dogfood probes do not overlap with cron/oven-tender spawning duplicate downloads.
#
# Usage:
#   ./scripts/wait-for-vllm.sh
#   CHUMP_VLLM_MODELS_URL=http://127.0.0.1:8001/v1/models ./scripts/wait-for-vllm.sh
#   CHUMP_WAIT_VLLM_TIMEOUT_SECS=3600 ./scripts/wait-for-vllm.sh   # first HF pull can exceed 20 min
#
# Environment:
