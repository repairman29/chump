...
#
# Schedule: launchd/cron (see scripts/doc-keeper.plist.example), or CI. Logs: logs/doc-keeper.log
...
if [[ "${DOC_KEEPER_CHECK_LINKS:-1}" != "0" ]]; then
  if command -v python3 &>/dev/null; then
    ...
    set +e
    link_out=$(python3 "$ROOT/scripts/doc-keeper-check-links.py" "$ROOT" "${scan_roots[@]}" 2>&1)
    link_rc=$?
    set -e
    if [[ -n "$link_out" ]]; then
