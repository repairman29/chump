...
- `chump pr explain-block <PR>` observability — no ambient events emitted (read-only diagnostic, no state mutation), `gh` REST call cost (not GraphQL), 6-outcome failure-class taxonomy (local/sibling_blocked/fleet_wide/green/pending/hard-error), smoke test command: [`docs/process/PR_EXPLAIN_BLOCK_OBSERVABILITY.md`](./docs/process/PR_EXPLAIN_BLOCK_OBSERVABILITY.md) (INFRA-1647)
