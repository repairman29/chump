...
| preregistration content (INFRA-113, 2026-04-28) | pre-commit | closing an `EVAL-*` or `RESEARCH-*` gap whose preregistration is missing required sections / sample size / non-Anthropic judge / A/A baseline / effect threshold / RESEARCH_INTEGRITY pointer | `CHUMP_PREREG_CONTENT_CHECK=0` with justification | file existence wasn't enough — a one-line prereg passed RESEARCH-019 but violated RESEARCH_INTEGRITY.md; test: `scripts/ci/test-prereg-content-guard.sh` |
