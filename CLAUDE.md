...
- **CSS token discipline (INFRA-1590, [`docs/process/CSS_TOKEN_DISCIPLINE.md`](./docs/process/CSS_TOKEN_DISCIPLINE.md)).** Pre-commit gate rejects raw hex/rgb/hsl colors in CSS property values, non-canonical `--*-primary` / `--*-secondary` variable definitions, mismatched `var()` fallbacks vs `:root` values, and non-canonical hex spreading to >3 files. Canonical tokens: `--text`, `--text-secondary`, `--bg`, `--bg-surface`, `--bg-elevated`, `--accent`, `--accent-dim`, `--success`, `--warn`, `--error`, `--border`, `--radius`, `--radius-sm`. Bypass: `Token-Discipline-Bypass: <reason>` commit trailer (emits `kind=token_discipline_bypass` to ambient). Disable: `CHUMP_CSS_TOKEN_CHECK=0`.
