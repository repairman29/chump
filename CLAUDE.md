...
- **Auto-merge IS the default** (since INFRA-MERGE-QUEUE, 2026-04-19). `bot-merge.sh --auto-merge` arms `gh pr merge --auto --squash` at PR creation — **BUT ONLY if all required CI checks are passing** (see **CI pre-flight gate** below). The GitHub merge queue rebases each PR onto current `main` and re-runs CI before the atomic squash, so commits aren't lost and stale-base merges can't happen. See `docs/MERGE_QUEUE_SETUP.md`.
