...
- **Uncommitted work is the ONE class git cannot recover — commit early, and never work in a shared primary checkout (RESILIENT-256).** 2026-08-09: an agent ran `git reset --hard origin/main` in the shared almanac primary checkout and destroyed ~574 insertions across 7 files that had sat uncommitted for 13 hours, plus a second session's in-flight work. Recovery was **impossible** — unstaged changes never enter the object store, so there is no reflog entry, no stash, and `git fsck --lost-found` over every dangling object finds nothing. Git has **no pre-reset hook**, so the durable fix is that bytes reach the object store EARLY, not that the command gets caught. Three layers, in order of load-bearing-ness: (1) **`chump wip-snapshot [--dir D]`** writes a dirty tree to `refs/wip/<branch>/<epoch>` **without touching the working tree or index** — safe against a checkout another session is typing into; the `dev.chump.wip-watchdog` launchd job (`scripts/ops/uncommitted-wip-watchdog.sh`, every 30min, graded as `wip` by the reaper heartbeat watchdog) does this automatically for any checkout idle >3h and ALERTs `kind=wip_unprotected` / `wip_snapshotted`. (2) **`chump git-guard`** runs as a PreToolUse hook and **refuses** `reset --hard` / `checkout -- .` / `restore` / `clean -f` / `worktree remove --force` against a dirty tree, naming every file it would destroy; it also refuses Edit/Write into a PRIMARY checkout that live linked worktrees share. Bypasses: `CHUMP_ALLOW_DESTRUCTIVE_GIT=1` (audited, `kind=destructive_git_override`) and `CHUMP_PRIMARY_WRITE_GUARD=warn|off`. Wired repo-locally in `.claude/settings.json`; fleet-wide via `scripts/setup/install-git-guard-hook.sh` (the loss happened in almanac, not here). (3) **`scripts/dev/own-worktree.sh <slug>`** — one command for your own worktree, in any repo. The primary checkout is **read-mostly**.
