...
- **System-2 reasoning peel (Phase 4.1):** When **`CHUMP_THINKING_XML`** is on, the system prompt asks for optional **`<plan>`** and required **`<thinking>`** before tool calls; **`thinking_strip::peel_plan_and_thinking_for_tools`** and **`agent_loop`** peel them for tool handling and monologue logging. Set **`CHUMP_THINKING_XML=0`** for brittle cloud models. See **`docs/ROADMAP_CLAUDE_UPGRADE.md`** (Phase 4) and **`.env.example`**.
