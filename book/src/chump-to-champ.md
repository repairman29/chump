...
### 2.2b Curriculum bootstrap (INFRA-1615)

- **What it does:** `chump-brain/skills/` (the on-disk skill store backing `memory_type=skill`) is per-host and gitignored — correct, since it's learning state, not source. But that means a freshly installed fleet starts with zero skills until enough session-observed patterns accumulate, which can take weeks. `skills-bundle/<name>/SKILL.md` at the repo root is the fix: a small, **committed** set of operator-curated seed skills (today: `verify-existence`, `claim-without-collision`, `pre-ship-ci-prediction`), each with a full user-story-shaped `SKILL.md` body. `chump init` calls `crate::skills::install_bundle()`, which copies every `skills-bundle/<name>/SKILL.md` into `$CHUMP_BRAIN_PATH/skills/<name>/SKILL.md` (default `chump-brain/skills/`). The copy is idempotent — a name that already exists on disk is left alone, so an operator's locally-edited copy is never overwritten. Re-run standalone at any time (e.g. on an existing install where `chump init` already ran) via `chump skill bundle install`.
- **Adding more seed skills:** drop a new `skills-bundle/<name>/SKILL.md` in a normal PR — the next `chump init` or `chump skill bundle install` picks it up automatically, no code change required.
- **Gap vs. theory:** the bundle is a fixed operator-curated starting curriculum, not a learned or auto-distilled one. There is no mechanism yet for a fleet to nominate a locally-learned skill back into `skills-bundle/` for other installs to inherit.
