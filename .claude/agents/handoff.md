...
**Before** the standard work-your-lane protocol, arm a real-time watcher on your own session inbox so wizard/operator/sibling-curator dispatches wake you immediately (0s lag) instead of waiting for the next cron tick. See [`docs/process/INBOX_WATCHER_PATTERN.md`](../../docs/process/INBOX_WATCHER_PATTERN.md) for the harness-agnostic contract.
