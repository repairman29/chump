...
**This file is the single source of truth for what to work on.** Doc index: [`docs/README.md`](https://github.com/repairman29/chump/blob/main/docs/README.md). Heartbeat (work, opportunity, cursor_improve rounds), the Discord bot, and Cursor agents should read this file—and `docs/briefs/CHUMP_PROJECT_BRIEF.md` for focus and conventions—to know what they're doing. Do not invent your own roadmap; pick from the unchecked items below, from the task queue, or from codebase scans (TODOs, clippy, tests).
...
**Architecture vision:** For cognitive architecture roadmap, empirical status, and frontier research direction, see [`docs/strategy/CHUMP_TO_CHAMP.md`](https://github.com/repairman29/chump/blob/main/docs/strategy/CHUMP_TO_CHAMP.md).
...
- **Chump (heartbeat / Discord):** In work rounds, use the task queue first; when the queue is empty or in opportunity/cursor_improve rounds, read this file and `docs/briefs/CHUMP_PROJECT_BRIEF.md`, then create tasks or do work from the unchecked items.
- **Cursor (when Chump delegates or you're in this repo):** Read this file and `docs/briefs/CHUMP_PROJECT_BRIEF.md` when starting. Pick implementation work from the roadmap priorities or from the prompt Chump gave you. Align with conventions in CHUMP_PROJECT_BRIEF and `.cursor/rules/`.
...
- Keep the stack healthy: Ollama, embed server, battle QA self-heal, autonomy tests. **Run the roles in the background:** Farmer Brown, Heartbeat Shepherd, Memory Keeper, Sentinel, Oven Tender (Chump Menu → Roles tab; schedule with launchd/cron per docs/operations/OPERATIONS.md).
- **Fleet expansion:** Chump external work, research rounds, review round; Mabel watch rounds; Scout/PWA as primary interface — see [`docs/architecture/FLEET_ROLES.md`](https://github.com/repairman29/chump/blob/main/docs/architecture/FLEET_ROLES.md).
- **Long-term vision:** In-process inference (mistral.rs), eBPF observability, managed browser (Firecrawl), stateless task decomposition, JIT WASM tools — see [`docs/strategy/CHUMP_TO_CHAMP.md`](https://github.com/repairman29/chump/blob/main/docs/strategy/CHUMP_TO_CHAMP.md) for the frontier roadmap.
...
**Goal:** Make Chump **reliable, reachable, governable, context-rich, and polished** enough to serve as a **primary execution layer** (overcome “hobby stack” limits). **Authoritative pillar backlog and acceptance criteria:** [ROADMAP_UNIVERSAL_POWER.md](https://github.com/repairman29/chump/blob/main/docs/strategy/ROADMAP_UNIVERSAL_POWER.md) (items **P1.x–P5.x**).

**Rollup — check a box when that pillar’s exit criteria in that doc are met:**

- [x] **P1 — Reliability boring** — green-path + preflight + CI + degraded UX matrix + **`turn_error` hints** + **local OpenAI retry/circuit doc** (**P1.5–P1.6** done in [ROADMAP_UNIVERSAL_POWER.md](https://github.com/repairman29/chump/blob/main/docs/strategy/ROADMAP_UNIVERSAL_POWER.md)).
- [x] **P2 — Reach** — **P2.1–P2.5** shipped in [ROADMAP_UNIVERSAL_POWER.md](https://github.com/repairman29/chump/blob/main/docs/strategy/ROADMAP_UNIVERSAL_POWER.md) (Web Push MVP, async jobs, webhook hardening, cron snippets, repo profiles). **Stretch:** **P2.6** remote runner RFC/MVP.
...
- [x] **P4 — Compounding context** — **P4.1–P4.5** shipped ([CONTEXT_PRECEDENCE.md](https://github.com/repairman29/chump/blob/main/docs/architecture/CONTEXT_PRECEDENCE.md), session limits doc, optional LLM e2e flag, task spine hints, COS decisions API + PWA). **Optional:** automated long-thread soak.
...
**Execution order:** P1 → P2 → P3 → P4 → P5 (see dependency notes in [ROADMAP_UNIVERSAL_POWER.md](https://github.com/repairman29/chump/blob/main/docs/strategy/ROADMAP_UNIVERSAL_POWER.md)).
...
|--------------|---------------------------|--------------|
| Policy-driven cascade, privacy, regimes | P1–P4, [PROVIDER_CASCADE.md](https://github.com/repairman29/chump/blob/main/docs/architecture/PROVIDER_CASCADE.md), [CONTEXT_PRECEDENCE.md](https://github.com/repairman29/chump/blob/main/docs/architecture/CONTEXT_PRECEDENCE.md) | Keep green; extend only with metrics when changing defaults. |
| Speculative rollback ≠ file/HTTP undo | [TRUST_SPECULATIVE_ROLLBACK.md](https://github.com/repairman29/chump/blob/main/docs/architecture/TRUST_SPECULATIVE_ROLLBACK.md), [ADR-001](https://github.com/repairman29/chump/blob/main/docs/architecture/ADR-001-transactional-tool-speculation.md), `sandbox_tool` | Prefer **sandbox / git worktrees** for reversible file work; do not imply full transactional side effects. |
...
| Inference wall time dominates UX | [PERFORMANCE.md](https://github.com/repairman29/chump/blob/main/docs/operations/PERFORMANCE.md), [STEADY_RUN.md](https://github.com/repairman29/chump/blob/main/docs/operations/STEADY_RUN.md), [INFERENCE_STABILITY.md](https://github.com/repairman29/chump/blob/main/docs/operations/INFERENCE_STABILITY.md), `CHUMP_LIGHT_CONTEXT` | **Latency envelope** below; hardware/model path is primary lever—document baseline before arguing “fast enough.” |
| Consciousness adds latency; utility unclear | [`docs/strategy/CHUMP_TO_CHAMP.md`](https://github.com/repairman29/chump/blob/main/docs/strategy/CHUMP_TO_CHAMP.md), A/B harness in ROADMAP “Chump-to-Champ” | **Utility pass** below (same tasks, on vs off). |
| One operator, intermittent use | Phase 2 blinds, daily driver | **Blinds + 95-step plan** are the corrective—[PRODUCT_REALITY_CHECK.md](https://github.com/repairman29/chump/blob/main/docs/strategy/PRODUCT_REALITY_CHECK.md) for review hygiene. |

**Unchecked proof work (pick in order; do not skip P5 while inventing new “consciousness” features):**

- [x] **Latency envelope (daily driver):** Measured and documented in [LATENCY_ENVELOPE.md](https://github.com/repairman29/chump/blob/main/docs/operations/LATENCY_ENVELOPE.md). Tool-free fast path + schema compaction + KV cache keep-alive: **26s → 0.5s** (warm cache) on qwen2.5:7b Ollama. Three optimization layers: `compact_tools_for_light()`, `message_likely_needs_tools()` with `response_wanted_tools()` auto-retry, `keep_alive=30m`. See [PERFORMANCE.md](https://github.com/repairman29/chump/blob/main/docs/operations/PERFORMANCE.md) §8.
...
- [x] **Overnight / 72h soak:** Run all roles + primary surface for 72h. Capture **pre/post**: SQLite size/WAL pattern, model server restarts, `logs/` growth, and `GET /api/stack-status` samples; append findings to [INFERENCE_STABILITY.md](https://github.com/repairman29/chump/blob/main/docs/operations/INFERENCE_STABILITY.md) §Soak. Infrastructure shipped via INFRA-350: `scripts/setup/install-soak-checkpoint-launchd.sh` + `docs/SOAK_72H_LOG.md`; T0=2026-05-03T04:21:07Z.
...
- [x] Define Chump–Cursor communication protocol and direct API contract: roles, shared context, message types, lifecycle (docs/process/CHUMP_CURSOR_PROTOCOL.md); expand CURSOR_CLI_INTEGRATION.md with prompt format, timeouts, and API contract for future HTTP bridge.
...
- [x] GitHub: add repo to CHUMP_GITHUB_REPOS, set GITHUB_TOKEN; Chump can list issues, create branches, open PRs. Documented in .env.example, docs/operations/OPERATIONS.md "Push to Chump repo", docs/process/AUTONOMOUS_PR_WORKFLOW.md.
...
Baseline docs: [EXTERNAL_GOLDEN_PATH.md](./getting-started.md), [PRODUCT_CRITIQUE.md](https://github.com/repairman29/chump/blob/main/docs/strategy/PRODUCT_CRITIQUE.md), [ONBOARDING_FRICTION_LOG.md](https://github.com/repairman29/chump/blob/main/docs/process/ONBOARDING_FRICTION_LOG.md). README quick start must stay aligned with the golden path.
...
Living map of an external strategy paper vs this repo: [EXTERNAL_PLAN_ALIGNMENT.md](https://github.com/repairman29/chump/blob/main/docs/strategy/EXTERNAL_PLAN_ALIGNMENT.md). Granular work packages (WP-IDs), priorities, and completion rules: [HIGH_ASSURANCE_AGENT_PHASES.md](https://github.com/repairman29/chump/blob/main/docs/strategy/HIGH_ASSURANCE_AGENT_PHASES.md) (**§3** includes **WP-1.4** matrix + **WP-1.5** multimodal RFC **Proposed**). Theme order defaults to **inference/ops → pilot kit → fleet transport → research/RFCs**. Optional in-process depth: [`docs/architecture/MISTRALRS.md`](https://github.com/repairman29/chump/blob/main/docs/architecture/MISTRALRS.md).

- [x] **Alignment doc + pilot repro kit + inference RFC skeleton:** [EXTERNAL_PLAN_ALIGNMENT.md](https://github.com/repairman29/chump/blob/main/docs/strategy/EXTERNAL_PLAN_ALIGNMENT.md), [DEFENSE_PILOT_REPRO_KIT.md](https://github.com/repairman29/chump/blob/main/docs/operations/DEFENSE_PILOT_REPRO_KIT.md), [rfcs/RFC-inference-backends.md](https://github.com/repairman29/chump/blob/main/docs/rfcs/RFC-inference-backends.md).
- [x] **Inference hardening (ops + UX):** Extend [INFERENCE_STABILITY.md](https://github.com/repairman29/chump/blob/main/docs/operations/INFERENCE_STABILITY.md) and [OPERATIONS.md](./operations.md) with a **degraded-mode** playbook (MLX OOM symptoms, Ollama fallback, when `farmer-brown` applies); ensure browser/PWA surfaces `stack-status` `inference.error` where users already load stack status (e.g. Providers/settings flows) when `models_reachable === false`.
...
- [x] **Fleet transport spike:** Design note under [`docs/architecture/FLEET_ROLES.md`](https://github.com/repairman29/chump/blob/main/docs/architecture/FLEET_ROLES.md) or [ROADMAP_MABEL_DRIVER.md](https://github.com/repairman29/chump/blob/main/docs/strategy/ROADMAP_MABEL_DRIVER.md) + time-boxed prototype — **outbound** WebSocket or MQTT over Tailscale from Pixel to Mac; Mac **pauses** sentinel-delegated repair when peer last-seen exceeds threshold (no infinite wait).
- [x] **WASM tool lane:** Extend [WASM_TOOLS.md](https://github.com/repairman29/chump/blob/main/docs/architecture/WASM_TOOLS.md) with a “new sandboxed tool” checklist; explicit **non-goal** near term: WASM-wrapping all of `run_cli`.
- [x] **High-assurance agent architecture (paper → phases):** [HIGH_ASSURANCE_AGENT_PHASES.md](https://github.com/repairman29/chump/blob/main/docs/strategy/HIGH_ASSURANCE_AGENT_PHASES.md) — **§3 master registry** (WP-1.1 … **WP-1.4** … WP-8.1), **§4 handoff template**, **§17** when to check this box. Rule: **one WP-ID per Cursor run**; set WP **Status** to **Done** in §3 when merged. **Closed under §17 strict (2026-04-09):** all **P0** WPs **2.2**, **3.1**, **4.1** are **Done** in §3. (Use **§17 loose** if you later reopen the umbrella until Phases 1–5 are materially complete—document in a follow-up PR.)
...
- [x] **Phase 2 — Safety:** Heuristic risk for run_cli (and optional write_file); CHUMP_TOOLS_ASK; approval flow with ToolApprovalRequest; one approval UX (Discord + Web); audit logging (tool_approval_audit in chump.log). OPERATIONS.md "Tool approval", docs/operations/TOOL_APPROVAL.md, ARCHITECTURE.md "Tool policy (allow / deny / ask)".
...
- [x] Task routing (assignee): task_db assignee column (chump/mabel/jeff/any); task tool create/list; context_assembly "Tasks for Jeff". See docs/architecture/FLEET_ROLES.md.
...
Master vision and detail: [`docs/strategy/CHUMP_TO_CHAMP.md`](https://github.com/repairman29/chump/blob/main/docs/strategy/CHUMP_TO_CHAMP.md). Research brief for external review: [`docs/briefs/CHUMP_RESEARCH_BRIEF.md`](https://github.com/repairman29/chump/blob/main/docs/briefs/CHUMP_RESEARCH_BRIEF.md).

**Section 1 — Harden and measure (near-term)**

- [x] **Metric definitions** (`docs/operations/METRICS.md`): CIS, Turn Duration, Auto-approve Rate, Phi Proxy, Surprisal Threshold — exact computation from DB/logs.
...
- [x] **Speculative execution** (`speculative_execution.rs`, wired from `agent_loop` for ≥3 tools/batch): snapshots belief_state, neuromod, full blackboard; `evaluate()` uses surprisal **EMA delta since fork** plus confidence and failure ratio; `rollback()` restores in-process state only. See `docs/architecture/ADR-001-transactional-tool-speculation.md`. Tests in `speculative_execution` + integration coverage.
...
Full doc index: [`docs/README.md`](https://github.com/repairman29/chump/blob/main/docs/README.md). Key references: [`docs/strategy/CHUMP_TO_CHAMP.md`](https://github.com/repairman29/chump/blob/main/docs/strategy/CHUMP_TO_CHAMP.md) (architecture vision, empirical status, and frontier roadmap), [`docs/briefs/CHUMP_PROJECT_BRIEF.md`](https://github.com/repairman29/chump/blob/main/docs/briefs/CHUMP_PROJECT_BRIEF.md) (focus and conventions), [`docs/architecture/FLEET_ROLES.md`](https://github.com/repairman29/chump/blob/main/docs/architecture/FLEET_ROLES.md), [`docs/architecture/RUST_INFRASTRUCTURE.md`](https://github.com/repairman29/chump/blob/main/docs/architecture/RUST_INFRASTRUCTURE.md) (Tower, tracing, proc macro, inventory, typestate, pool, notify), [`docs/process/EXTERNAL_GOLDEN_PATH.md`](https://github.com/repairman29/chump/blob/main/docs/process/EXTERNAL_GOLDEN_PATH.md) (external adoption), [`docs/research/CONSCIOUSNESS_AB_RESULTS.md`](https://github.com/repairman29/chump/blob/main/docs/research/CONSCIOUSNESS_AB_RESULTS.md) (A/B study data), [`docs/research/consciousness-framework-paper.md`](https://github.com/repairman29/chump/blob/main/docs/research/consciousness-framework-paper.md) (research paper with Scaffolding U-curve + neuromod findings).
