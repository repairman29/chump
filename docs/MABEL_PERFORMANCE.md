...
| **Android build fails** (e.g. `ring`: "failed to find aarch64-linux-android-clang") | Build was run without NDK env (e.g. raw `cargo build --target aarch64-linux-android`). | Use **`./scripts/build-android.sh`** only; it sets CC, linker, and `ANDROID_TARGET_DIR`. Deploy scripts call it. See [OPERATIONS.md](OPERATIONS.md) "Mabel deployment issues". |
