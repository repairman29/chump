...
This doc is the **canonical** handoff surface for “how we work together safely.” Older references to `docs/CHUMP_CURSOR_PROTOCOL.md` / `docs/CURSOR_CLI_INTEGRATION.md` should be updated to point here until those files are restored from archive.

---

## 1. Non-negotiables (same as every Chump agent)

Read **`docs/AGENT_COORDINATION.md`** and **`docs/AGENT_LOOP.md`** before parallel work:

- **`scripts/gap-preflight.sh <GAP-ID>`** then **`scripts/gap-claim.sh <GAP-ID>`** before touching shared paths or `docs/gaps.yaml`.
...
- **Gap filing under contention:** follow **AGENT_LOOP § Go slow to go fast** — `scripts/broadcast.sh`, PR diff scans for reserved IDs, and prefer **`scripts/gap-reserve.sh`** (see **INFRA-021**) over ad-hoc “next free number” picks.
...
When Chump (e.g. Discord) delegates coding to Cursor, the pattern is **non-interactive `agent`** with an explicit prompt and scope — see **`docs/INTENT_ACTION_PATTERNS.md`** (“Use Cursor to fix …” → `run_cli` with `agent -p "..."`).
