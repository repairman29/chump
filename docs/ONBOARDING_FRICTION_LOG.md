...
| 2026-04-17 | agent | `scripts/bot-merge.sh --auto-merge` didn't exist yet — agents used manual `git push + gh pr create` | `bot-merge.sh --auto-merge` added and documented in CLAUDE.md | Fixed |
...
| 2026-04-19 | new-external | `chump --preflight` binary not found after `cargo build` (PATH not updated) | Golden path now uses `./target/debug/chump --preflight`; also `./scripts/chump-preflight.sh` | Fixed |
