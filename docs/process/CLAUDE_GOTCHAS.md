...
**Active target/ directory pruning (INFRA-1349, 2026-05-15).** Unlike the stale-worktree-reaper which removes entire worktrees, the **target-dir-reaper** prunes the `target/` directory in *active* worktrees when disk pressure is high or the worktree has been idle too long. This keeps disk usage bounded without destroying in-flight work.

1. **`scripts/coord/target-dir-reaper.sh`** — default is **dry-run** (safe to run anytime). With **`--execute`**, it removes `target/` directories from worktrees under `.claude/worktrees/` and `/tmp/chump-*` when:
   - Free disk is **< 20%** (configurable via `CHUMP_REAPER_DISK_PCT`), OR
   - Worktree is idle **> 6 hours** (configurable via `CHUMP_REAPER_IDLE_HOURS`)
   - AND the worktree has **no active lease** (safety guard — never touches in-use worktrees)
2. **macOS — expected setup for dogfooding:** run **`scripts/setup/install-target-dir-reaper-launchd.sh`** once per machine so the reaper runs **every 30 min**. **Verify:** `launchctl list | grep dev.chump.target-dir-reaper`. **Logs:** `/tmp/chump-target-dir-reaper.{out,err}.log`. **Disable:** `launchctl unload ~/Library/LaunchAgents/dev.chump.target-dir-reaper.plist`.
3. **Ambient events:** emits `kind=target_artifact_reaped` (per-artifact with `freed_gb`, `age_hours`) and `kind=target_dir_reaper_summary` (aggregate stats).

**Complementary tools:** `cargo-target-reaper.sh` (INFRA-1250) prunes stale cargo fingerprints and fleet-wide shared caches; `target-dir-reaper.sh` (INFRA-1349) prunes worktree-local `target/` directories when active.
