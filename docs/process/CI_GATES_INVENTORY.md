...
| docs-delta linked worktree regression (INFRA-1474) | INFRA-1521 — `test-docs-delta-worktree.sh` creates a real `git worktree add` linked worktree and drives a real `git commit` through the commit-msg hook via `core.hooksPath`; exercises worktree gitdir-pointer-file semantics that don't exist inside preflight's own checkout |
| docs-delta linked worktree smoke test (INFRA-1521) | INFRA-1521 — `test-docs-delta-linked-worktree.sh`, same shape as the row above: spins a temp linked worktree + real `git commit` to prove the commit-msg hook reads `$1` correctly outside the main checkout |
