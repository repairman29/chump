...
| **`mlx-community/Qwen3.5-9B-OptiQ-4bit`** | **Qwen3.5 9B** (reasoning, Metal) | **Reasoning model — not recommended for chat default** (PRODUCT-024). Emits ~50 s of plain-prose Chain-of-Thought before any user-visible answer; measured **56.985 s** for a 4-character `pong` reply. Useful only once **INFRA-184** routes plain-prose CoT to `thinking_delta`. For chat latency, use the **Default** row above (`Qwen2.5-14B-Instruct-4bit`) or the **3bit** variants below. Still text-generation (`pipeline_tag: text-generation`); avoid the `Qwen3.5-9B-MLX-4bit` / `Qwen3.5-9B-4bit` VLM repos. |
...
**Caution — Qwen3.5 MLX repos:** Many **`mlx-community/Qwen3.5-*-MLX-4bit`** builds are tagged **image-text-to-text** / **mlx-vlm** conversions. For **plain Chump chat + tools**, use **`pipeline_tag: text-generation`** repos. Note that the Qwen3.5 family produces long internal Chain-of-Thought (see **PRODUCT-024** / **INFRA-184**); for chat-latency-sensitive use, prefer **Qwen2.5-Instruct** (default row above) until plain-prose CoT routing lands.

**Switching the running MLX server to a chat-friendly model (PRODUCT-024 helper):** if a session loaded a reasoning model and chat is unusably slow, run **`scripts/setup/use-chat-model.sh`** to atomically stop :8000 and restart with **`Qwen2.5-14B-Instruct-4bit`** (override with `--model …` for a different non-reasoning model). The helper prints a `Say pong` probe time so you can verify the swap actually fixed latency (target: under 5 s end-to-end).
