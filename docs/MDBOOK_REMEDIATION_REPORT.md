...
| Area | Outcome |
|------|---------|
| **Publish map** | `docs/MD_BOOK_PUBLISH_SURFACE.md` documents nav chapters, sync mapping, local preview, and common failure modes. |
...
| **Roadmap links** | `scripts/roadmap-mdbook-links.py` rewrites `docs/ROADMAP.md` targets to GitHub blob URLs or in-nav `./chapter.md` links so the synced roadmap chapter does not emit dead local `.html` neighbors on Pages. |
| **Doc hygiene** | Published and adjacent docs were updated for **mistral.rs** consolidation (`docs/MISTRALRS.md` replaces removed split/matrix filenames in cross-links). |
...
1. Edit the **canonical** file under `docs/` (see the sync table in `docs/MD_BOOK_PUBLISH_SURFACE.md`).
2. For large `docs/ROADMAP.md` link edits: `python3 scripts/roadmap-mdbook-links.py`.
...
- `docs/MD_BOOK_PUBLISH_SURFACE.md` — publish surface and sync ownership.
