...
| `install-stale-worktree-reaper-launchd.sh` | Install hourly launchd plist `dev.chump.stale-worktree-reaper` that calls `stale-worktree-reaper.sh --execute`. Idempotent. Mirrors `install-roles-launchd.sh` pattern. |
...
| Script | Description |
|--------|-------------|
| `print-repo-metrics.sh` | Emit **wc** LOC for `src/**/*.rs`, `cargo test -p chump -- --list` count, `docs/**/*.md` count (`--json`); paste into reviews — [PRODUCT_REALITY_CHECK.md](../docs/strategy/PRODUCT_REALITY_CHECK.md). |
...
| `append-result.sh` | Append a scored run result to `docs/research/CONSCIOUSNESS_AB_RESULTS.md`. |
