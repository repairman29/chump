...
| 2026-04-14 | **Cognitive hot-path integration** — Closed the Active Inference perception-action loop in `agent_loop.rs`: (1) `update_tool_belief()` + `record_prediction()` called after every tool result; (2) `decay_turn()` per iteration; (3) EFE-based tool ordering via `efe_order_tool_calls()` + `epsilon_greedy_select()`; (4) precision-weighted surprisal (confidence amplifies/dampens prediction errors); (5) neuromod-aware fast path (serotonin modulates tool-skip threshold); (6) `check_regime_change()` posts to blackboard after tool turns. Total overhead <1ms per tool call. |
