...
| 7 | Metacognition | `src/belief_state.rs`, `src/neuromodulation.rs`, `chump-neuromodulation` crate | EVAL-026 cross-architecture neuromod **harm** signal -0.10 to -0.16; EVAL-043 ablation flags shipped (`CHUMP_BYPASS_BELIEF_STATE`, `CHUMP_BYPASS_NEUROMOD`); **EVAL-048 (2026-04-20):** sweep harness confirmed working (`scripts/ab-harness/run-ablation-sweep.py`), noise floor delta=0.0 (expected); **EVAL-063 (2026-04-20):** LLM-judge re-score n=50/cell — belief_state delta=+0.020, surprisal delta=+0.000, neuromod delta=+0.040, all CIs overlap — NO SIGNAL across all three modules — see `docs/eval/EVAL-049-binary-ablation.md` (EVAL-063 Re-score section) | COVERED+VALIDATED(NULL) — **REMOVAL-001 decisions:** (1) **surprisal_ema: REMOVED** — delta=0.000 exactly; `src/surprise_tracker.rs` deleted per REMOVAL-002 (2026-04-21). (2) **belief_state: REMOVE** — delta=+0.020, no positive signal; REMOVAL-003 filed. (3) **neuromodulation: KEEP + re-test** — NULL on Llama-70B (EVAL-063) and qwen14B (EVAL-069); haiku-specific bypass test not yet run; F1 U-curve predicts possible mid-tier harm; REMOVAL-004 filed (haiku neuromod retest, P3). EVAL-026 −0.15 prior not reproduced under EVAL-060 on non-haiku agents. |
| 8 | Executive Function | `src/agent_loop/`, `src/blackboard.rs`, `src/tool_middleware.rs`, `chump-coord` crate | **EVAL-058 (2026-04-20):** `CHUMP_BYPASS_BLACKBOARD` flag shipped; binary-mode n=30/cell sweep: Cell A acc=0.100 CI [0.035, 0.256], Cell B acc=0.067 CI [0.018, 0.213], delta=−0.033, CIs overlap — NO SIGNAL. **EVAL-064 re-score (2026-04-20, LLM judge, n=50/cell):** Llama-3.3-70B via Together.ai, Cell A acc=0.900 CI [0.786, 0.957], Cell B acc=0.960 CI [0.865, 0.989], delta=+0.060, CIs overlap — NO SIGNAL. See `docs/eval/EVAL-058-executive-function-ablation.md`. | COVERED+VALIDATED(NULL) — **REMOVAL-001 decision: KEEP.** EVAL-064: delta=+0.060 (CIs overlap, NEUTRAL per criterion) — only module in the NULL set with a directional positive trend. Single-turn fixture likely underestimates value; cross-turn state tracking requires multi-turn entity-rich eval (INFRA-008) for definitive signal. |
...
- `CHUMP_BYPASS_SURPRISAL=1` — surprisal EMA bypass (removed per REMOVAL-002 2026-04-21;
...
`docs/process/RESEARCH_INTEGRITY.md`, and all three scorer methods (heuristic, liberal judge, strict judge) fail to
...
- A/B results stack: [`docs/research/CONSCIOUSNESS_AB_RESULTS.md`](./CONSCIOUSNESS_AB_RESULTS.md)
