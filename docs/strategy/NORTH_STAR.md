...
The cognitive architecture underneath Chump — free energy, surprise tracking, neuromodulation, precision weighting, memory graphs, counterfactual reasoning — is the design hypothesis we are betting on, not a validated mechanism. It is what we believe will make a small agent stay grounded, coherent, and useful over time without a human watching every step. Each module is an answer to that question that we are testing, not asserting. Some early directions have been positive (task-class-aware lessons block); others have been net-negative or null pending further work (neuromodulation cross-architecture; belief-state removed pending ablation revival; surprisal/full architecture gated on subsequent ablations). See [`docs/process/RESEARCH_INTEGRITY.md`](../process/RESEARCH_INTEGRITY.md) for what is currently validated, what is prohibited from being claimed, and which gaps gate which claims.
...
- **Cognition-stack pilot A/B** (2026-05-10) — first A/B of `CHUMP_CONSCIOUSNESS_ENABLED=1` vs `0` returned null at the preregistered threshold. The combined cognition stack did not measurably move agent task outcomes in this configuration. Specifics live in the private companion repo.
- **Validated narrower direction to date** — instruction injection has tier-dependent effects: prescriptive lessons help small models on specific tasks and harm frontier models. Per-module status is in the public faculty stub at [`docs/architecture/CHUMP_FACULTY_MAP.md`](../architecture/CHUMP_FACULTY_MAP.md) (specific findings live in the private research repo per [`docs/agents/RESEARCH_PRIVACY.md`](../agents/RESEARCH_PRIVACY.md)).
- **Follow-up.** A P0 confirmation A/B is filed to repeat at higher power with an explicit dependency map showing which downstream gaps reopen if positive vs stay closed if null. Until that returns a result, the strategic framing above stands; if the confirmation also returns null, this document gets rewritten honestly.
