...
- **What shipped:** `CHUMP_TOGETHER_JOB_REF` required before `together:` harness calls; `CHUMP_TOGETHER_CLOUD=1` + same ref for `run-study*.sh` and `run-live-ablation.sh` Together path; doc [`docs/operations/TOGETHER_SPEND.md`](./TOGETHER_SPEND.md) (budget request template + env table).
- **Lane B:** after merge, paste approved ticket into `CHUMP_TOGETHER_JOB_REF` before any preregistered Together judge sweep; aligns with weekly budget checkpoint in [`docs/research/RESEARCH_EXECUTION_LANES.md`](./RESEARCH_EXECUTION_LANES.md).
