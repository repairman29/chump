...
**Related:** [`docs/eval/preregistered/COST_OPTIMIZATION.md`](./eval/preregistered/COST_OPTIMIZATION.md) (Together free-tier substitutions), [`docs/process/RESEARCH_INTEGRITY.md`](./RESEARCH_INTEGRITY.md) (judge-family rules), [`docs/operations/TOGETHER_SPEND.md`](./TOGETHER_SPEND.md) (budget ticket + env gates before paid Together runs).
...
- [x] **Per-family dry run** — commands for **n=1–5** pilots in the batch sheet; Anthropic path runnable without Together; Together-backed families **blocked on `TOGETHER_API_KEY`** (see [`docs/research/RESEARCH_AGENT_REVIEW_LOG.md`](./RESEARCH_AGENT_REVIEW_LOG.md)).
