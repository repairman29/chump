...
**Tool-use visibility:**
**Best in class.** Every tool call shown as a collapsible card: `read_file docs/NORTH_STAR.md`, `bash git status`, `write_file src/main.rs`. User can approve/reject each call. Auto-approve mode available (off by default). Detailed diff view for file writes.
...
The following principles are synthesized from the competitive scan and the North Star doc (`docs/NORTH_STAR.md`). PRODUCT-012 should treat these as constraints, not suggestions.
