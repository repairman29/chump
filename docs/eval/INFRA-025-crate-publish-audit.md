...
**Date:** 2026-04-22  
**Scope:** Workspace members from root `Cargo.toml` `[workspace].members`, plus the root package **`chump`** (binary `chump`). Out of scope for this pass: `examples/*`, `wasm/*`, `repos/*`, `tests/mock_projects/*` (not workspace members).
...
| Monolith (root app) | 1 | **`chump`** — publishes once all in-tree `chump-*` crates are on crates.io at the versions pinned in root `Cargo.toml` (dual `path` + `version` deps). |
...
| `rust-agent` (third-party) | Yes — **unrelated** crate at `0.0.5`; not this repo (historical naming collision only) |
| `chump` (this repo) | Free at audit time — intended crates.io name for the root package |
...
|-------|------|-------|---------|----------|------------|------------------|---------|----------------|
| `chump` | `.` | **P** (app) | MIT | yes | yes | — | **Pass when** all pinned `chump-*` versions exist on crates.io | Free (this product) |
...
Previously failed `cargo publish` before every in-tree dependency had a **registry `version`** alongside `path` in root `Cargo.toml`. With pins present, **`cargo publish -p chump --dry-run`** should succeed once each `chump-*` crate at the pinned version is on crates.io.
...
5. **Root `chump`** — last, after all library crates above are published at pinned versions  
