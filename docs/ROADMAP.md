...
- [ ] **P5 — Product polish** — **P5.3** parity matrix; **P5.4** turn_error copy; **P5.1** onboarding (partial: PWA bar + Settings + step track — [PWA_ONBOARDING_WIZARD.md](PWA_ONBOARDING_WIZARD.md)); **P5.2** mobile pass (matrix + touch CSS — [UI_MANUAL_TEST_MATRIX_20.md](UI_MANUAL_TEST_MATRIX_20.md)); **P5.5** signed/notarized distribution.
...
- [ ] **Latency envelope (daily driver):** Define N (e.g. 10) and record **median + p90** wall time for (a) one no-tool chat turn, (b) a **3-tool** sequence, on the **golden-path** model and machine you actually use. **Procedure + table template:** [LATENCY_ENVELOPE.md](LATENCY_ENVELOPE.md). Use `./scripts/mlx-warmup-chat.sh`, `CHUMP_LOG_TIMING=1`, and/or [scripts/golden-path-timing.sh](../scripts/golden-path-timing.sh) as appropriate; paste **dated** measured rows into that doc or [ONBOARDING_FRICTION_LOG.md](ONBOARDING_FRICTION_LOG.md).
